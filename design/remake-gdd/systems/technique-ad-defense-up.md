...
祈導師三层池均按原版顺序含 `AD`。`REMAKE-033/037` 之后池内不再抽签，选哪一项以及选哪个
友军由共享专家效用决定，不再使用 AD 自己的“缺失生命最多”原生选择器；完整规则见
[`expert-enemy-ai.md`](expert-enemy-ai.md#与逐技术规格的关系)。四步选择范围、不排除自己、
满生命或冰封单位、对焦后显示组 16“防禦提昇.”、相同 165 tick 时间线与原子提交都不变；
已有防御提升的目标记为“无效／重复”，因此不会被重复施加。

AD 的支援估值是目标的 `防御 + 30`，低于同池治疗与恢复在有缺失生命时的价值，所以编队
带伤时祈導師先治疗，全员健康时才轮到 AD。规划不读取模拟 PRNG，只有本动作的经验仍消费
既有随机调用；画面和提示不消费模拟随机数。`legacyStrict` 可保留原版抽签与原生选择器。
...
6. AI 按受益者的 `防御 + 30` 支援价值选择目标，同分再比目标威胁；已有防御提升记为重复
   而不再施加；使用组 16 原文；
