...
邪法師第三层池只有 `4F`，所以可选集合与原版一致。`REMAKE-033/037` 之后目标由共享专家
效用选择，不再使用原版评分；完整规则见
[`expert-enemy-ai.md`](expert-enemy-ai.md#与逐技术规格的关系)。统一距离 7、排除冰封敌军、
对焦准备目标、组 10 原文、同一三段表现与同一准备结果都不变。第一／二层不使用 `4F`；
