...
- 战役准备器在进入部署前应用 stage 1 的证据驱动职业覆盖；提交后把准备后的 roster
  与部署结果一起交给战斗工厂。
- DOM 绘制名单、页签、人数、错误和焦点，并发送语义动作。
- Phaser 只投影地图、固定单位、`FFh` 空位和当前落点，不拥有增删合法性。
...
- 鼠标直接点击剩余 open cell：只改变 `currentOpenCell`，不自动加入单位。
- 完成：五至八人均合法；不要求填满空位。
...
DOM 名单保留三列五行和 `Ⅰ/Ⅱ/Ⅲ/結束` 的原版信息结构，显示人物名、职业、等级、
生命与状态。地图保留像素棋子和部署格；当前 open cell 同时用轮廓和位置反馈，不能只靠
颜色。键盘焦点、鼠标悬浮和手柄焦点都映射同一个 `focus`。减少动态、浏览器缩放、
