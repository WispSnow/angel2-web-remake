...
进入可执行子集；`SM` 原始异常保留在证据和未来兼容边界，不映射成 `OJ`。
`REMAKE-033/037` 之后池内不再抽签，选哪一项以及选哪个中心由共享专家效用决定，也不再
使用 `3I` 自己的“缺失生命最多、平局取线性格较后者”原生选择器；完整规则见
[`expert-enemy-ai.md`](expert-enemy-ai.md#与逐技术规格的关系)。距离 6 的中心选择范围、
排除冰封接受者、组 14“生命全.”、共同表现与同一准备／提交路径都不变；无合法中心时仍
进入既有职业 fallback。

与原版不同的是，专家效用按环内全部接受者的实际恢复量合计估值，所以 `3I` 会选覆盖
总缺失生命最多的中心，并在只有单名伤员时输给同池的单体 `2H`；全员满生命时整项记为
“无效／重复”而不会被选中。`legacyStrict` 可保留原版抽签与原生选择器。
...
5. AI 按环内合计恢复量选择中心，能覆盖更多伤员的中心优先；全员满生命时整项记为无效；
   对白为组 14“生命全.”；
