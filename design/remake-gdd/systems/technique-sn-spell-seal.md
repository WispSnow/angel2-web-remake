...
第三层咒術師的原版 `1H/SA/LA/IP/SN` 五槽池继续界定 SN 只在第三层可选。
`REMAKE-033/037` 之后池内不再抽签，选哪一项以及选哪个目标由共享专家效用决定，也不再
使用 SN 自己的“有效防御最低、当前生命最低”原生排序；完整规则见
[`expert-enemy-ai.md`](expert-enemy-ai.md#与逐技术规格的关系)。距离 7、龍可选但只免疫
状态写入、依 `REMAKE-014` 对焦并显示“禁咒.”，以及共用正式表现与结算都不变。SN 对技术
职业目标的控制估值高于同池的 SA／IP，对非技术职业则很低，因此实际是否出 SN 取决于场上
是否有技术职业敌人。side 1 自动第三层咒術師使用同一规划器；第一／二层不纳入 SN。
`legacyStrict` 可保留原版抽签与原生排序。
...
6. 冰封目标可受禁咒，冰壳高于封印图形；
7. 场上存在技术职业敌人时，敌方第三层专家规划选择 SN，按共享效用选敌并显示“禁咒.”；
