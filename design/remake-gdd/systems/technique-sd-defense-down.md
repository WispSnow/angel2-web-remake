...
计数重置为 3，使其有效防御固定减少 20；防御提升与防御下降可同时存在并相消。
...
敌方魔祭師保持原版三层池。抽到 SD 时，在当前位置距离 4 内选择有效防御最低、再取生命
最低的敌军；冰封目标不因 SD 而过滤。成功施术依 `REMAKE-014` 对焦并显示“防禦降低.”，
随后共用玩家结算与表现。side 1 自动魔祭師也把 SD 纳入对应层级池。
...
4. 有效防御减少 20，防升／防降并存相消，普通伤害与地形防御均使用该值；计数完整轮递减，TR 可清除；
5. 10 个 `2×2` 描述符依次覆盖帧 `1..40`，总计 150 tick，开头仅一次 `E/8`；
6. 冻结目标完整播放并正常获得状态，冰壳高于四格破盾图形，生命和防魔不变；
7. 敌方 AI 原生池可确定性抽中 SD，以低防御／低生命规则选敌并显示“防禦降低.”；
