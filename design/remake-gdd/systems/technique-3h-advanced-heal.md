...
进入可执行子集；后续动作完成时再恢复完整池。AI 抽中 `3H` 后在距离 7 内选缺失生命最多的
同阵营未冰封单位，平局取线性格较后者；满血仍可选。成功时使用组 15 原文、共同
表现和同一准备／提交路径；无合法目标时进入职业 fallback，不改选其他池项且不消耗本次治疗随机。
...
5. AI 在两个相同缺失生命候选中选线性格较后者，使用组 15“生命單.”，且不先移动；
