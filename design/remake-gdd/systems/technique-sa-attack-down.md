...
敌方咒術師保持原版三层池。抽到 SA 时，在当前位置距离 4 内选择有效防御最低、再取生命
最低的敌军；冰封目标不因 SA 而过滤。成功施术依 `REMAKE-014` 对焦并显示“功擊降低.”，
随后共用玩家结算与表现。side 1 自动咒術師把 SA 纳入其已实现动作池；尚未实施的 SN 仍
保留原生槽位并走普通行动兜底，不压缩敌方随机池。
...
7. 敌方 AI 原生池可确定性抽中 SA，以低防御／低生命规则选敌并显示“功擊降低.”；
