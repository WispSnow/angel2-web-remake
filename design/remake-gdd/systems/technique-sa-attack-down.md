...
计数重置为 3，使其有效攻击减少 20；攻击提升与攻击下降可同时存在并相消。难度
...
- `[OF]` `1000:8C2D` 的 `-20` 排在最高难度加成 `1000:8BD1` 之前，两者都作用于同一个
  有效攻击字，因此难度 3 的 side 2 目标实得 `-30`；side 1 与其余三档难度仍是 `-20`。
  装载顺序见
...
4. 有效攻击减少 20（「無法無天」的敌方为 30），攻升／攻降并存相消，计数在完整轮边界
   递减且 TR 可清除；
