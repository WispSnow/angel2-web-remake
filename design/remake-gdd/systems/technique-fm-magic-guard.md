...
- `[DD]` `REMAKE-026` 在 `stableRemake` 保留池顺序，但抽到 `FM` 时统一使用玩家动作定义
  ...
  对焦准备目标，但不显示虚构台词。
- `[DD]` `REMAKE-033/037` 取代了上一条中的“抽签”与“最大缺血／后格优先选择器”：池顺序
  仍界定候选集合，但选哪一项与选哪个友军改由共享专家效用决定，已有防魔记为重复。共用
  玩家动作定义、目标合法性、对焦与“不显示虚构台词”不变；详见
  [`expert-enemy-ai.md`](expert-enemy-ai.md#与逐技术规格的关系)。
...
第三层魔導師池按原版顺序为 `3H/2I/AA/FM`。`REMAKE-033/037` 之后池内不再抽签，选哪一项
以及选哪个友军由共享专家效用决定，不再使用 FM 自己的“缺失生命最多”原生选择器；完整
规则见 [`expert-enemy-ai.md`](expert-enemy-ai.md#与逐技术规格的关系)。七步选择范围、
对焦目标、不显示对白、同一 300 tick 时间线与原子提交都不变；已有防魔的目标记为
“无效／重复”，因此不会被重复施加。

FM 的支援估值是固定 `120`，与目标属性无关，所以同分由目标威胁打破——编队全员健康时，
威胁最高的往往是施法者自己，第三层魔導師因此会先给自己上防魔。只有当某名友军的
`攻击 + 40` 超过 `120` 时同池的 AA 才会压过 FM，而缺失生命足够多时 `3H/2I` 又会压过
两者。以上只适用于 side 1 独立友军与“自由行动”：依 `REMAKE-139`，side 2 施放的 FM 一律
记为“无效／重复”候选——敌方阶段之后紧接回合边界，计数为 1 的防魔在任何玩家魔法到来之前
就清零，敌方魔導師因此改在 `3H/2I/AA`、移动、休息与待命之间选择，不再每回合原地给自己
重放一道从未生效的防魔。规划不读取模拟 PRNG，只有本动作的经验仍消费既有随机调用；镜头、
声音和画面不消费模拟随机。`legacyStrict` 只保留孤项证据与原版抽签，并在缺行项被选中时安全进入普通 AI
兜底，不模拟残留内存。
...
| 配置 | 行为 |
| --- | --- |
| `stableRemake` | 玩家／双方 AI 统一距离 7 和友方动作定义；冰封友军可接受；AI 无对白；side 2 的 FM 依 `REMAKE-139` 记为无效候选 |
...
7. 编队全员满生命时，第三层 side 1 自动单位的专家规划确定性选择 FM，使用距离 7、按目标
   威胁取施法者自身，且不显示虚构对白；side 2 的第三层魔導師依 `REMAKE-139` 不再提交 FM，
   改选 AA、治疗、移动或待命；
...
- 2026-09-02：`REMAKE-139` 把 side 2 的 FM 记为无效候选。`expert-ai.test.ts` 断言 side 2 施法者
  的 FM 效用为 `waste`、side 1 仍为 `120` 支援；竞技场敌方 FM 用例改为断言敌方第三层魔導師
  不再提交 FM。
