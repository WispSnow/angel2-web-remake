...
[`web-remake-rule-decisions.md#remake-140敌方施放的防魔撑到下一玩家阶段ai-不给自己施放防魔`](../../../reverse/gdd/web-remake-rule-decisions.md#remake-140敌方施放的防魔撑到下一玩家阶段ai-不给自己施放防魔)
...
依 `REMAKE-140`，敌方施放时写入 2，使它撑过紧接的回合边界、在下一玩家阶段仍然有效。
...
- `[DD]` `REMAKE-140`：原版统一在下一玩家轮边界递减，敌方阶段末写入的防魔在玩家出手前就
  清零。`stableRemake` 改为按施放方阶段位置写入计数：side 1 仍写 1，side 2 写 2，两侧的
  防魔都恰好覆盖一个完整的对方阶段；消费仍直接写 0，刷新只写对应值、不叠加。同一决定让双方
  自动规划不再以任何魔導師（含施法者自己）为 FM 候选，玩家手动施放仍可选自己与其他魔導師。
...
2. 从当前状态完整复制目标状态，仅把 `magicGuard` 重置为 side 1 的 1 或 side 2 的 2
   （`REMAKE-140`）；其余状态、生命和冰封不变；
...
6. 后续魔法效果按各动作已闭合顺序读取／消费防魔；若未消费，每次 `startNextRound` 饱和
   减一：side 1 的 1 在紧接的边界清零，side 2 的 2 先减为 1、撑过下一玩家阶段后才清零。
...
FM 的支援估值是固定 `120`，与目标属性无关，所以同分由目标威胁打破。依 `REMAKE-140`，
魔導師（含施法者自己）不是自动规划的候选：编队全员健康时威胁最高的往往正是施法者，原本它
会每次自己的防魔失效就原地再给自己放一次、不管远处友军，两名魔導師站在一起则会每回合互相
加；现在防魔只落在射程内威胁最高、尚未防魔的非魔導師同伴身上，射程内没有这样的同伴时不产生
FM 候选，魔導師转向其他候选、跟随前线或待命。只有当
某名友军的 `攻击 + 40` 超过 `120` 时同池的 AA 才会压过 FM，而缺失生命足够多时 `3H/2I` 又会
压过两者。`REMAKE-140` 同时让 side 2 的防魔真正能挡住下一玩家阶段的魔法，已有防魔（计数
`>0`）记为重复，同一同伴最多每两个完整回合刷新一次。`REMAKE-139` 曾把 side 2 的 FM 记为
无效候选，已由 `REMAKE-140` 撤回。规划不读取模拟 PRNG，只有本动作的经验仍消费既有随机
调用；镜头、声音和画面不消费模拟随机。`legacyStrict` 只保留孤项证据与原版抽签，并在缺行项被选中时安全进入普通 AI
兜底，不模拟残留内存。
...
| 配置 | 行为 |
| --- | --- |
| `stableRemake` | 玩家／双方 AI 统一距离 7 和友方动作定义；冰封友军可接受；AI 无对白；side 2 施放写计数 2、撑到下一玩家阶段，自动规划不把防魔加在魔導師身上（`REMAKE-140`） |
...
3. 成功后只把 `magicGuard` 重置为 side 1 的 1 或 side 2 的 2，每次取得 `10..13` 并消费一次
   PRNG；取消不消费；
4. side 1 未使用的防魔在下一完整轮边界变 0；side 2 的先变 1、再于其后的边界变 0；存读档
   保持状态和后续随机流；
5. 代表性炎暴／落雷／冰雪、魔弓、普通物理和 `WD` 用例分别保持既有挡伤／消费合同；
6. 冰封友军成功获得防魔且仍不可行动；之后范围魔法覆盖时完整跳过并保留防魔；
7. 编队全员满生命且有非魔導師同伴在射程内时，第三层双方专家规划确定性选择 FM，使用距离 7、
   按目标威胁取该同伴，且不显示虚构对白；施法者自己与其他魔導師不是候选；敌方施放的防魔在
   回到玩家阶段时仍为 1；
...
- 2026-09-02：`REMAKE-140` 把 side 2 施放的防魔计数改为 2（取代同日 `REMAKE-139` 的无效
  候选条款），并让自动规划不给自己施放。`actions.test.ts` 断言 side 2 施放写 2、边界后为 1、
  再一边界清零且刷新不叠加，side 1 仍为 1→0；`expert-ai.test.ts` 断言以魔導師为目标的 FM
  记为无效、射程内只有另一名魔導師时改为跟进移动；竞技场敌方第三层魔導師用例断言防魔落在士兵护卫身上、写入 2 且回到
  玩家阶段时仍为 1。
