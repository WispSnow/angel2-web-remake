...
- `[H]` 标题停留期间 `MUSIC/1` 循环播放：证据只登记它的明确停止点，`MUSIC/14` 另外明记“自然
  播完即停”，该不对称被读作 RIX 驱动重复 `MUSIC/1`。待试玩确认，见 `REMAKE-112`。
- `[DD]` 难度菜单取消不中断 `MUSIC/1`：原版取消也发送 RIX 停止并让标题此后静音，复刻版只在
  确认难度、真正离开启动流程时停止音乐，见 `REMAKE-112`。
