...
| `S00-UI-00b` | 标题分页记录读取 | 标题选择“繼續遊戲” | “讀取遊戲進度”、原版五列表头、20 个手动槽（每页五槽）及所选槽的关卡/规则集/保存时间摘要 | 上下选槽、左右翻页、主操作读取、`Esc` 返回；空槽/坏档可高亮但不得离开此状态 | 恢复玩家阶段、恢复下一关路由或返回标题 |
...
| `S00-UI-04` | 普通玩家阶段 | `SAY/0001` 结束 | `10×7` 战场、焦点；空地悬浮显示明亮战术桌/实时小地图/标记，点击空地后右栏上半显示职业相关地形特性并保留下方小地图；单位格在变暗且无标记的小地图上叠加详情，身份区只显示居中的职业／名字并保留 HP/EXP 双竖条；点击单位后，控制／行动、友军／敌军战术及力场摘要改在底部字幕栏按需显示；地点、回合；首次可有非阻断提示 | 方向、主操作、副操作、系统、目标、集体命令 | 阶段提交或终局 |
| `S00-UI-04a` | 系统根菜单 | 玩家阶段中性态呼出 | 原文五项“遊戲功能／勝利條件／讀取記錄／儲存記錄／離開遊戲”；背景保留原右栏焦点态 | 导航/主操作选择；副操作返回 | 返回战场、打开对应子表面 |
| `S00-UI-04b` | 游戏功能/战中记录/退出确认 | 从系统根菜单选择 | 游戏功能严格显示“人物圖像／戰鬥動畫／地圖方格／地圖捲動／ＡＩ對話”五项 `ON/OFF`；战斗动画首次为 `ON`；记录使用 20 个分页手动槽；退出使用 `D/46` 肖像、原文与确认菜单 | 游戏功能上下循环、主操作切换、副操作返回；记录上下选槽、左右翻页；反馈逐字时首次主操作只补全文字 | 返回根菜单、恢复战场快照或退出 |
| `S00-UI-05` | 胜利条件 | 目标动作或系统菜单选择 | 准确胜利/失败文本；背景战斗暂停接收指令 | 主操作或副操作关闭 | 返回原战场状态 |
| `S00-UI-06` | 行动菜单/范围 | 选中合法单位或动作 | 选中单位先显示职业菜单且不显示范围；基础菜单为“移动／攻击／休息”。确认移动后才显示范围；合法格全亮、非法格按 `11h/44h` 网点压暗。移动后切换为“攻击／结束／返悔” | 上下/悬停选择菜单，主操作确认，副操作逐层返回；范围态使用方向/指针选格 | 提交动作或回到中性战场 |
| `S00-UI-06a` | 集体命令/命令对白/撤退确认 | 玩家阶段中性态呼出 | “全部休息／跟随主将／自由行动／全面撤退”；前三项确认后由妮雅在上方窗口显示对应原文，逐字结束后才结算；跟随主将要求焦点位于尚未行动的我方单位；撤退使用 `D/46`、原文和原版确认菜单 | 导航、主操作确认、副操作取消；逐字时首次主操作只补全文字；前三项对白无第二次确认并自动执行 | 我方自动阶段、重建本关或返回战场 |
...
- `[DD]` 五项系统根菜单，以及胜利存档、全面撤退和离开游戏的“確 定／取 消”选择，
  ...
  产品构图的表面不套用通用菜单外框。
- `[DD]` 战中“儲存記錄／讀取記錄”与战后“儲存遊戲進度”两个记录槽表面也套用同一
  `A/0001` 外框，使其与相邻的系统根菜单、集体命令和确认菜单在同一视觉族内；外高仍
  按 `24N + 28` 计算，战中记录选单的 N 是“标题列＋五个槽位列＋分页列＋取消列＝8”
  （`220 px`），战后存档没有取消列，N＝7（`196 px`）。分页是 Web 侧二十槽需求新增
  的一列，按同一 `24 px` 行距排版，不改动外框几何。槽位与分页只降字级承载数据：
  槽位列为 `16 px` 槽号栏＋可省略关卡名栏＋固定回合栏，长关卡名截断后回合数仍可见。
  记录槽 `[OF]` 的原版五列构图只保留在标题“讀取遊戲進度”表面，本条不改变它。
...
- `[DD]` 通用外框的红橙棋盘纹理只有 50% 不透明度，覆盖右侧战术栏会透出栏内文字；
  ...
  `480 px` 之内：战后存档槽列表 `(336,100)`，胜利存档、全面撤退与离开游戏三处共用的
  两项确认菜单 `(336,110)`，战中记录选单直接占用系统根菜单原位 `(304,47)`。
...
第 0 关复刻默认保留原版发布版键位习惯，同时允许未来重映射：

- 方向：方向键或 `W/Z/A/S`；斜向为 `Home/PageUp/End/PageDown`；
- 主操作：`Ctrl/Insert/Space` 或鼠标左键；
- 次操作：`Alt/Delete/Enter` 或手柄副按钮；
- 鼠标右键：当前存在移动/目标/行动菜单或可取消面板时先逐层取消并消费该次输入；仅在中性战场循环对焦下一名尚未行动的我方单位；
- `Esc` 系统菜单，`Tab` 集体命令；
- `[OF]` `E/M` 分别打开“音效開關”四项分类面板与五档音乐音量面板；
...
| `secondary` | 取消键 | 明确的返回按钮 | 东侧按钮 | 逐层取消；关闭目标面板 |
| `nextUnactedAlly` | 可重映射 | 中性战场右键 | 可重映射肩键 | 按固定我方编队顺序循环并跳过已行动单位；只更新焦点和镜头；若右键已用于逐层取消，本动作不得继续执行 |
...
移动或攻击候选态中，原版黑白斜面候选框必须随指针经过的战场格即时移动，但悬浮本身不提交动作。蓝/红单位圆圈仍标记当前已选单位；确认移动后，候选框固定在最终目标格直至逐格动画结束，不得在路径前方逐格跳动。
...
15. `Esc` 独占系统菜单键盘快捷入口；`Alt/Delete/Enter` 只负责取消/返回，不在中性战场打开系统菜单。鼠标右键不显示浏览器菜单：有活动操作或面板时只取消当前层并消费事件，中性战场才按固定我方编队顺序循环对焦下一名尚未行动单位；循环一周不改变棋盘、行动位或 PRNG。
...
21. 移动/攻击范围打开后，鼠标悬浮候选格会移动黄色候选框而不改变单位坐标或行动状态；点击合法格才提交。移动动画期间候选框保持在最终目的地，单位圆圈与角色沿实际路径更新。
...
以上状态已映射到 [`startup.spec.ts`](../../../tests/e2e/startup.spec.ts) 的 `BOOT-A/BOOT-B/BOOT-C`、[`tests/e2e/stage0.spec.ts`](../../../tests/e2e/stage0.spec.ts) 的 `S00-A` 至 `S00-N`，并由 [`stage0-real-clear.spec.ts`](../../../tests/e2e/stage0-real-clear.spec.ts) 的 `S00-O` 在无 `?test=1`、无调试 API 条件下完成真实通关。浏览器测试同时保存标题记录第 1/4 页、标题第 4 页、读档后战场、战中记录第 4 页、战后保存第 4 页、空地战术右栏、单位详情覆盖、EXP 非零、系统菜单、退出/撤退反馈、全景战斗、结算完成和窄屏等代表性截图，并对最终 stage 1 路由做黄金截图比较；第 0 关已由用户完成手动视觉与手感验收。
