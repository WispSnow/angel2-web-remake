...
以上状态已映射到 [`tests/e2e/stage0.spec.ts`](../../../tests/e2e/stage0.spec.ts) 的 `S00-A` 至 `S00-N`。浏览器测试同时保存空地战术右栏、单位详情覆盖、EXP 非零、系统菜单、战中记录、退出/撤退反馈、全景战斗、结算完成和窄屏等代表性截图；当前状态等待无夹具完整通关与用户手动视觉、手感验收。
