...
| `S00-UI-06` | 行动菜单/范围 | 选中合法单位或动作 | 选中单位先显示职业菜单且不显示范围；鼠标／触控选取时不播放虚拟手形滑行。基础菜单为“移动／攻击／休息”。确认移动后才显示范围；合法格全亮、非法格按 `11h/44h` 网点压暗。移动后落点有攻击目标时切换为“攻击／结束／返悔”，没有目标时为“確定／取消”（取消回原格重新选落点）；范围态右栏跟随光标：空格显示实时小地图（可悬停／点击／拖曳切换视口），单位格显示该单位详情 | 上下/悬停选择菜单，主操作确认，副操作逐层返回；范围态使用方向/指针选格 | 提交动作或回到中性战场 |
| `S00-UI-06a` | 集体命令/命令对白/撤退确认 | 玩家阶段中性态呼出 | “全部休息／跟随主将／自由行动／全面撤退”；全部休息与自由行动按 `REMAKE-122` 固定由本关主将（本关是妮雅）在上方窗口发出，跟随主将由玩家刚指定的临时主将发出，逐字结束后才结算；跟随主将要求焦点位于仍在场的我方玩家单位，已行动或行动禁用均可作为锚点；撤退使用 `D/46`、原文和原版确认菜单 | 导航、主操作确认、副操作取消；逐字时首次主操作只补全文字；前三项对白无第二次确认并自动执行 | 我方自动阶段、重建本关或返回战场 |
...
- `[OF]`/`[DD]` 新增两列必须描述同一关。原版存档头把关卡编号写在 `1Ah`（DS:`2E77`，
  模块 29 的“当前关卡”），把回合号写在 `1Ch`（DS:`2F83`；即时胜利 999 在构造文件元数据
  时序列化成 1000），战后存档发生在刚打完的那一关内部——下一关编号写的是另一个 GO
  共享字段 DS:`0008`，DS:`2E77` 不跟着前进——所以原版这一对字段始终是“关卡 N ＋ 已保存
  胜利”（`reverse/notes/save-slot-format.md` 的槽位元数据表、`BAT-043`）。复刻版的
  `CompletedSaveData` 以“下一关入口”为身份，`stageId`／`stageLabel` 存的都是目的地，
  因此显示层按目的地反查来源关卡：`[DD]` 战后记录一律显示刚打完那一关的关卡名，回合列
  显示“完”（1000 的复刻写法），战中记录显示当前关卡名与当前回合号。存档字段本身不改，
  标题记录列表的无障碍摘要沿用同一反查。
...
- `[DD]` Web 在整个 `640×350` 逻辑画面（标题、剧情、右栏、菜单和战场内部）使用
  原版手指，战场四边按上述原版箭头切换；不再显示系统箭头、十字或 resize 光标，也不为
  移动、攻击和禁用按钮虚构原版未提供的额外状态图。这些图示由画面内指针绘制，与原版
  软件光标一样随游戏画面缩放；宿主游标只在指针尚未进入画面时作为后备，规则见
  `07-ui-ux-and-presentation.md` 的画面内指针段落。
- `[DD]` 唯一的例外是四角：复刻在四角两轴同时滚动，改用复刻自绘的四枚斜向白箭头，
  让箭头与滚动方向一致，规则与造型见 `07-ui-ux-and-presentation.md` 的边缘滚屏段落。
...
- `P`／`Pause` 或逻辑画布外「暫停」按钮冻结整个游戏；暂停层内 `P`／`Pause`／`Esc` 恢复且不得把同一输入送回战场；
...
| `pause` | `P`／`Pause` | 逻辑画布外「暫停」 | 不要求默认绑定 | 冻结模拟、表现与声音；恢复不提交底层动作 |
...
11. 点击尚未行动的基础职业单位后先出现“移动／攻击／休息”菜单且范围为空；选择移动才显示合法范围；完成移动后，落点旁有敌军时菜单变为“攻击／结束／返悔”，没有目标时为“確定／取消”。
...
14b. 鼠标移到空地以恢复战术桌时，只改变右栏表现光标，不清除战斗层最近一次明确选择的单位焦点；从地图物件打开集体命令后，“跟随主将”必须沿用该焦点判定可用性和主将身份。该焦点只在不属于玩家军团或已经离场时禁用；依 `REMAKE-131`，已行动或本轮行动禁用的单位仍可作为凝聚锚点。
14c. “跟随主将”的自动移动按完整可通行路线代价而非曼哈顿距离推进；墙体绕行允许暂时远离
    主将，无完整通路时不得用几何方向制造伪路径。每次实际移动仍按相邻格路径逐段播放。
...
16. 小地图悬停显示 `30×21` 白色预览框并夹取在 `150×150` 范围内；点击将焦点设为预览原点 `+4,+3` 并移动视口，但棋盘快照和 PRNG 完全不变。移動／攻擊／技術选格期间光标停在空格时小地图同样可用：点击只重定视口与光标，不改变已选单位与候选格；按住主键拖曳时视口连续跟随、光标在放开时才落到视口中心，拖曳中的指针不会触发战场悬停或边缘滚屏。
