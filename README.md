...
浏览器打开 `http://127.0.0.1:4173/`。推荐先用鼠标体验：左键选择/确认，右键循环对焦下一名尚未行动的我方单位，指针停留在战场边框或底部地点 banner 可滚动镜头。键盘方向为方向键或 `W/Z/A/S`，主操作为 `Ctrl/Insert/Space`，取消/返回为 `Alt/Delete/Enter`；`Esc` 是系统菜单的唯一键盘快捷键，`Tab` 打开集体命令，`F1–F4` 分别是全部休息、跟随主将、自由行动和全面撤退，`E` 打开四项音效设置，`M` 当前切换音乐。
...
没有单位焦点时，右栏显示战术桌与实时小地图；悬浮或选中单位时改为单位详情。`Esc` 根菜单提供原文五项“遊戲功能／勝利條件／讀取記錄／儲存記錄／離開遊戲”；表现速度、地图/全景战斗与音乐位于“遊戲功能”子菜单，“音效設定”继续进入原版“說話／移動／戰鬥／按鍵”四项独立开关。表现与声音选项不会改变模拟状态或随机数序列。
