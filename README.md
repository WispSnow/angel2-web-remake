...
的 20 槽 JSON 备份，用于在 Web 与桌面版之间迁移；同一排的「刪除」只清除光标所在的单个记录
（含损坏记录），确认框默认停在「取消」，删除后无法复原。
...
现有 20 槽备份导出／导入在 Web 与桌面版之间迁移。Windows 上这些数据位于
`%LOCALAPPDATA%\com.wispsnow.angel2-web-remake`（卸载时勾选删除应用数据清掉的就是它与
`%APPDATA%` 下的同名目录），是 WebView 的整库数据而不是逐槽文件，无法像原版那样删掉单个
`WAR0.TST`；删除单个记录请用游戏内记录面板的「刪除」。桌面窗口中的「銳利」与「平滑」会按客户区
