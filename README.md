...
页面可以指定攻方/守方职业、`≤10` 格挡或 `>10` 重伤、守方是否死亡、左右攻击方向、播放速度、循环和音效。时间轴支持暂停、40 ms 单步与按原生语义节点跳转；“戰士重傷／戰士死亡／弓兵重傷／弓兵死亡”四个按钮可直达当前重点验收组合。配置会同步到网址参数，便于复制和复现同一场景。

女帝没有独立的原版普通全景战斗图形：左侧资源为空，唯一可重放的右侧资料逐字节复用士兵画面。选择女帝时，实验室会自动锁定到右侧并显示这一原版边界；这不是素材加载失败。
