...
第 0–5 关与异世界之门的关前剧情、部署／固定准备、玩家回合、技能／事件夹具、一击胜利和
直接通关入口，并可选择四档难度。选择“逐关代表性成长”或“深层转职分支覆盖”时，还可在
页面顶部输入固定“每关成长”并点击“套用设定值”；第 N 关按 `N × 每关成长` 建立成长预算，
例如每关 120 时第 1 关预算为 120、第 5 关为 600。普通角色先确定性随机进入二阶职业；预算
跨过当前职业转职阈值时扣除阈值、随机选择合法下一职，并把余额继续投入新职业。因此第 5 关
界面显示的是最终职业余额，而不是直接写入 600。相同 URL 的随机路径可复现，且不修改正式
记录。“使用档案值”会恢复原夹具经验。场景内工具栏不提供经验改值或重启功能。
