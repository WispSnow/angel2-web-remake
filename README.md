...
和逐帧单步。当前可重放正式已实现的 `1F/1H/1C/1L` 与完整原版 `2L/3L/4L` 表现；其余
原版技术名称仍显示在清单中，但以“尚未实现”禁用。中高究级落雷在这里只消费原版
图形、音效和固定时间线，不开放战役规则、AI 或存档语义。时间轴到达总时长后会进入
“完成·无残留”并移除全部技能效果；终点前的末帧按原版最后一次绘制后的等待继续显示：
初級炎暴、初級冰雪和四档落雷为 100 ms，初級治疗为 150 ms，而不是瞬间消失或完成后
永久残留。
