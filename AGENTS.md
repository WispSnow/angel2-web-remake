...
- 有交互部署的新关卡必须分别登记“名单标志槽”和“地图固定 side 1 槽”：名单 UI 只按
  原版 `scenarioUnitFlags` 的非零槽分页，固定棋盘单位的标志为 0 时仍参战但不得生成名单项；
  禁止把全部固定部署位置直接拼入名单候选。
- 新关卡必须把“敌方增援／动态生成”作为独立证据项：交叉检查静态模板、逐关回合事件
  处理器、完整回合特殊生成链以及击杀替补／形态转换；即使结论为“无增援”，也必须在
  关卡合同与验收中明确记录已排除的通道，不能只根据开局阵容推断。
...
新增可玩关卡还必须在 `src/game/debug-roster-profiles.ts` 为“逐关代表性成长”和“深层
转职分支覆盖”显式登记该关，并继承此前剧情中已经必然发生的入队／职业变化；不得因漏配
而静默回退为全员士兵。关卡若把新的具名角色放进部署候选或固定我方编队，还必须在同一文件
的 `DEBUG_CAMPAIGN_MEMBERS` 登记该角色的入队关卡与战役入队职业：每关成长按
“（当前关卡序数 − 入队关卡序数）× 每关成长值”逐角色推算，漏登记的槽会停留在关卡模板
基线而不会成长。回归测试必须从 `STAGE_RUNTIME_MANIFEST` 枚举全部已实装关卡，校验两套
成长档均有配置、每个具名部署候选都有对应成员登记，并通过真实 debug 入口断言玩家可见的
职业文字与棋子。
