...
| `deployment-ui.ts`、`deployment-minimap.ts` 与 `deployment-lab.css` 的模块 27 原版部署构图：三列五行矩形、A/2 职业图、点阵文字、页签／结束、200×200 全图、黑白当前格、错误条，以及只在悬浮／键盘导航出现的丰富信息卡 | 部署规则仍由 `deployment.test.ts`、`deployment-session.test.ts` 覆盖；画面取证由 `input-ui.json` v6 与 `angel2-input-ui.mjs` 硬校验 | `deployment-lab.spec.ts` 的原生坐标／像素／悬浮／多输入／宽窄屏截图；正式关卡身份与内容继续由对应 `stageN.spec.ts` 覆盖 |
