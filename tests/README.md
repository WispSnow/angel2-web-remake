...
| `content/ai-technique-dialogue.ts` 的上下文台词与 `controller.ts` 的点选混亂路径：玩家点选被混亂我方单位时播 `DS:84BB[1Ch]` 台词、不开命令菜单、按自动规则行动并消耗行动 | `ai-technique-dialogue.test.ts` | `arena-ailments.spec.ts` 的「confused player unit answers a click」；敌方混亂行为仍见同文件的 LA 用例 |
