...
| `src/game/content/stage3.ts` 的 `untouchedExperience`：`REMAKE-109` 第 3 关被救援的槽 3／20／21 以士兵 `300` 入队、只对未被战役写过的槽成立、开场即进入授职队列 | `stage3-battle.test.ts` 的入队基线用例，v85 不追补见 `save.test.ts` | `stage3.spec.ts` 的 `S03-P` |
