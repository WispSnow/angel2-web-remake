...
| `scripts/generate-resource-manifest.mjs`、`resource-loader.ts`、`persistent-resource-cache.ts`、`staged-render-asset-cache.ts`、`dom-image-readiness.ts`、`music-resource-cache.ts`、`startup-music.ts`、逐关资源门与职业表现按需预载；`boot` 的全部 PNG 与两首开场音乐、以及当前关所需肖像分层必须在各自加载页内就绪，缓存层与实际 DOM 段的图片解码并发均不超过 6 | `resource-manifest.test.ts`、`persistent-resource-cache.test.ts`、`staged-render-asset-cache.test.ts`、`dom-image-readiness.test.ts`、`map-unit-loading.test.ts`、`portrait-assets.test.ts`、`music-resource-cache.test.ts`、`stage-runtime.test.ts` | `resource-loading.spec.ts` 的跨刷新缓存、开场音乐解码／用户激活／单请求截图、开场／肖像解码冻结与单请求、开场／第 0 关并行、字节进度、当前职业全景门、失败重试与逐关预取；结局／片尾段见 `stage49-ending.spec.ts` 的 `S49-I` 与 `stage38.spec.ts` 的 `S38-H` |
