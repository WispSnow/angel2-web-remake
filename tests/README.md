...
| `scripts/generate-full-combat-atlases.mjs`、`full-combat-atlas.ts` 与全景普通战斗职业／阵营图集 | `full-combat-atlas.test.ts`、`full-combat.test.ts` | `combat-lab.spec.ts`、`compendium.spec.ts` 与相关关卡全景战斗用例 |
| `scripts/generate-battle-sprite-atlases.mjs`、`battle-sprite-atlas.ts` 与地图命中／死亡、回合切换及逐关连续特效图集 | `battle-sprite-atlas.test.ts` | `battle-sprite-atlas.spec.ts` 的实际请求边界；`stage0.spec.ts`、`stage4.spec.ts`、`stage26.spec.ts` 的对应表现用例 |
| `scripts/generate-resource-manifest.mjs`、`resource-loader.ts`、`music-resource-cache.ts`、逐关资源门与职业表现按需预载 | `resource-manifest.test.ts`、`map-unit-loading.test.ts`、`music-resource-cache.test.ts`、`stage-runtime.test.ts` | `resource-loading.spec.ts` 的开场／第 0 关并行、字节进度、音乐单请求复用、当前职业全景门、失败重试与逐关预取；加载页视觉审计同文件 |
