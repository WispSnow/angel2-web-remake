...
| `persistent-resource-cache.ts`、`staged-render-asset-cache.ts`、`resource-loader.ts` 与正式 Phaser 场景的两层复用：清单版本／身份命名的 Cache Storage 跨刷新保存原始字节，当前表面再从响应建立对象 URL；源 URL 不得在资源门后再次请求，预取只保留当前后两包的内存压缩字节 | `persistent-resource-cache.test.ts`、`staged-render-asset-cache.test.ts`、`resource-manifest.test.ts` | `resource-loading.spec.ts` 的完整包刷新零二次 fetch，以及正式初級炎暴故障注入、地图／棋子／技能图集单请求与截图；实验室不建立资源门，不替代该用例 |
