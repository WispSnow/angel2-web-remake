...
| `scripts/generate-resource-manifest.mjs`、`resource-loader.ts`、逐关资源门与 Phaser 棋子按需预载 | `resource-manifest.test.ts`、`map-unit-loading.test.ts`、`stage-runtime.test.ts` | `resource-loading.spec.ts` 的开场／第 0 关并行、字节进度、失败重试与逐关预取；加载页视觉审计同文件 |
