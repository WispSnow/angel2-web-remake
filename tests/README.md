...
| `scripts/generate-resource-manifest.mjs`、`resource-loader.ts`、`staged-render-asset-cache.ts`、`music-resource-cache.ts`、逐关资源门与职业表现按需预载；`boot` 的全部 PNG 必须在加载页内解码 | `resource-manifest.test.ts`、`staged-render-asset-cache.test.ts`、`map-unit-loading.test.ts`、`music-resource-cache.test.ts`、`stage-runtime.test.ts` | `resource-loading.spec.ts` 的开场解码冻结／单请求、开场／第 0 关并行、字节进度、音乐单请求复用、当前职业全景门、失败重试与逐关预取；加载页及开场视觉审计同文件 |
