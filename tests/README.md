...
| 第 31 个可玩关卡（内部第 32 关）直接 1–16 人部署、SAY 63/64、菲伊魯茵／芙瑪羅妮与十六名共享专家追击静态联军、全灭目标、五通道无动态增援、v61/v60 与冻结 stage 33 路由 | `stage32-content.test.ts`、`stage32-battle.test.ts`、`save.test.ts`、`stage-runtime.test.ts`、`debug-roster-profiles.test.ts`、`construction-terrain-assets.test.ts` | `stage32.spec.ts`、`stage31.spec.ts`、`debug.spec.ts` |
