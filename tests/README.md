...
| 改动区域 | 首选单元测试 | 首选浏览器测试 |
| --- | --- | --- |
| `src-tauri/`、`package.json#desktop:*` 与 `.github/workflows/desktop-windows.yml` 的 Windows 桌面包边界：稳定应用标识、只消费审计后的 `release/`、Evergreen WebView2、当前用户 NSIS、受限窗口尺寸能力、手动／标签构建、私有 artifact 与 Cloudflare 分离；`src-tauri/icons/` 作为唯一图标来源——NSIS 安装／卸载程序图标，以及 `public/favicon.ico`、`public/icon-256.png` 与 `index.html` 的分页图标声明 | `desktop-packaging.test.ts` | Windows runner 构建 artifact；面向玩家发布前另做真实 Windows WebView2 标题至进关、音画、缩放、输入与存档人工验收 |
| `src/game/simulation/actions/`、动作数值、PRNG 与 `REMAKE-125` 范围技术经验（落雷仅击杀、冰雪成功位移单次分层值、踏击固定 5 加全部击杀奖励） | `actions.test.ts` | 对应 `arena-*.spec.ts` 技能族文件；魔術士正式战役提交另见 `stage1.spec.ts`，魔弓线路见 `arena-magic-archer-route.spec.ts` |
...
| `persistent-resource-cache.ts`、`staged-render-asset-cache.ts`、`stage-surface-image-assets.ts`、`native-ui-assets.ts`、`resource-loader.ts` 与正式 Phaser／DOM 场景的两层复用：清单版本／身份命名的 Cache Storage 跨刷新保存原始字节，当前表面再从响应建立对象 URL；源 URL 不得在资源门后再次请求，战场共用 DOM/CSS 图片及当前小地图／我方棋子有限预解码，预取只保留当前后两包的内存压缩字节 | `persistent-resource-cache.test.ts`、`staged-render-asset-cache.test.ts`、`stage-surface-image-assets.test.ts`、`resource-manifest.test.ts` | `resource-loading.spec.ts` 的完整包刷新零二次 fetch、正式战场／部署／转职 DOM 图片零原始 URL 请求，以及正式初級炎暴故障注入、地图／棋子／技能图集单请求与截图；实验室不建立资源门，不替代该用例 |
...
| `scripts/generate-resource-manifest.mjs`、`resource-loader.ts`、`persistent-resource-cache.ts`、`staged-render-asset-cache.ts`、`dom-image-readiness.ts`、`music-resource-cache.ts`、`sound-effect-cache.ts`、`startup-music.ts`、逐关资源门与职业表现按需预载；`boot` 的全部 PNG 与两首开场音乐、每关的 `audio:effects`、当前关所需肖像分层，以及有部署阶段的关卡所需的延后加载部署介面模块，必须在各自加载页内就绪；当前我方职业须沿合法转职边准备完整可达闭包，防止同关第二次转职出现 `__MISSING`，图片与音效解码并发均不超过 6 | `resource-manifest.test.ts`、`sound-effect-cache.test.ts`、`persistent-resource-cache.test.ts`、`staged-render-asset-cache.test.ts`、`dom-image-readiness.test.ts`、`map-unit-loading.test.ts`、`portrait-assets.test.ts`、`music-resource-cache.test.ts`、`stage-runtime.test.ts` | `resource-loading.spec.ts` 的跨刷新缓存、开场音乐解码／用户激活／单请求截图、开场／肖像解码冻结与单请求、开场／第 0 关并行、字节进度、当前职业全景与音效门、正式地图技能零 `media` 请求、部署介面模块与关卡运行模块随关卡资源门就绪（载入页收起后零程式／样式请求）、代表性关卡的表面从不请求 `/assets/original` 原始 URL、失败重试与逐关预取；同关多段转职的 Canvas 纹理由 `stage3.spec.ts` 的 `S03-S` 与 `stage8.spec.ts` 的 `S08-L` 覆盖；结局／片尾段见 `stage49-ending.spec.ts` 的 `S49-I` 与 `stage38.spec.ts` 的 `S38-H` |
...
| `map-unit-presentation.ts`、`BattleScene`、部署与转职的敌我分离棋子视觉锚点；只平移原尺寸 PNG 内的可见棋子，不移动逻辑格、生命数字或命中范围 | `map-unit-presentation.test.ts` | `class-showdown.spec.ts`、`deployment-lab.spec.ts`、`promotion-lab.spec.ts` 的锚点与截图用例 |
...
| `fixed-stage-battle.ts`、stage 3 保存清单与 `REMAKE-126`：原版 stage 3 跳过敌方难度经验播种，四档保持职业等级 1，无法无天仅追加 50% 属性倍率；v95 及仍可读取的更早战中档移除旧播种基线并保留战中经验／累计伤害 | `stage3-battle.test.ts` 的无法无天精确数值、`save.test.ts` 的当前 round-trip 与 v95／v94 迁移 | `stage3.spec.ts` 的 `S03-T` |
...
| `content/ai-technique-dialogue.ts` 的取得经验短句 `18h` 与 `controller.ts` 的三个触发点：普通攻击击杀（地图分支排在 `MAGIC/12` 之前、全景分支排在其后且反击窗口在先攻之前）与玩家「技術」提交后死亡扫描移除单位；数字按 `0000:EF56` 右对齐定宽五格。射击提交、`1N` 直连技術与 AI 技术入口没有窗口 | `ai-technique-dialogue.test.ts` 的「the experience window's numeric field」 | `experience-gain-line.spec.ts` |
...
| `reverse/tools/angel2-promotion-table.mjs`、`scripts/generate-stage0-runtime.mjs`、`ui.ts`／`styles.css` 的原版转职单排菜单、A/2 可变尺寸棋子的原比例居中与悬浮详情，以及 `promotion-lab-session.ts` 的转职触发阈值 | `promotion-lab.test.ts`、`promotion.test.ts`、`stage0-content.test.ts`、`stage-surface-image-assets.test.ts`、`resource-manifest.test.ts` | `promotion-lab.spec.ts`、`stage0.spec.ts` 的正式转职用例、`resource-loading.spec.ts` 的对象 URL 边界；入口另见 `debug.spec.ts` |
...
| `scripts/generate-objective-panel.mjs`、`content/objective-panel.ts` 与 `ui.ts`／`styles.css` 的原版勝利條件面板：`0000:BAA4` → `0000:B97C` → `12E7:0008` 的 `DS:1236` 本体 `(80,40,320,175)` 色 1、`DS:122C` 的 +16 硬阴影、`12E7:00E1` 用 `A/0006` 图磚横铺的上下缘带、左右各五道 `DS:1248` 边柱、四角遮罩装饰（`0000:D9FA` + `0000:D790`）、`(96,56)` 文字原点与 SAY 游标步幅，内容是 `DS:1273` 为该关指定的 SAY 记录逐行原文；面板不含复刻自撰内容，主／次操作皆可关闭 | 无独立模拟数值测试（面板是纯派生表现；生成器自身对模组映像与 `A/0006` 图磚逐项断言，并输出 `objective-panel-edge-top/bottom.png` 与 `objective-panel-corner.png`） | `objective-panel-layout.spec.ts`（几何、鋪排方式、`--native-objective-*-source`、绘制层序，以及 `REMAKE-123` 把右侧／下方三枚装饰各外移 1 px 后的贴角落点） |
...
| `deployment-ui.ts`、`deployment-minimap.ts` 与 `deployment-lab.css` 的模块 27 原版部署构图：`0000:056E` 先铺满全屏的色 7 底、三列五行矩形、A/2 可变尺寸职业图的原比例居中、点阵文字、页签／结束、200×200 全图、黑白当前格、错误条，以及只在悬浮／键盘导航出现的丰富信息卡 | 部署规则仍由 `deployment.test.ts`、`deployment-session.test.ts` 覆盖；画面取证由 `input-ui.json` v6 与 `angel2-input-ui.mjs` 硬校验 | `deployment-lab.spec.ts` 的原生坐标／像素／36 个我方职业棋子比例／悬浮／多输入／宽窄屏截图；正式关卡身份与内容继续由对应 `stageN.spec.ts` 覆盖——名单棋子图在命中框之外，按 `deployment-roster-figure-<index>` 定位，图档身分读 `data-source-url`（`src` 在资源包上线后是 `blob:`） |
| “遊戲功能”原版“子 選 單”的五项顺序、`ON/OFF`、面板／命中行几何、原版调色板与手形光标，以及键盘和鼠标切换 | 无独立模拟数值测试 | `game-functions-menu.spec.ts` |
| `src/game/scaling.ts`、`desktop-runtime.ts`、`display-settings.ts`、`host-chrome-radio.ts`、`interface-zoom.ts` 的宿主「畫面縮放」与桌面「介面縮放」偏好：Web 的最多 1 倍与整数倍留边；桌面客户区按宽高等比自适应、跨 DPR 整数因子、显示器边界及原生窗口目标；页面缩放倍率还原与整数倍的 `LogicalSize` 换算；宿主单选组的按键不得抵达战场；逻辑屏外选择器的位置与持久化 | `scaling.test.ts`、`desktop-runtime.test.ts`、`preferences.test.ts` 的「display preferences」、`desktop-packaging.test.ts` 的窗口与权限契约 | `display-scaling.spec.ts`，桌面视觉审计用 `VISUAL_AUDIT=1` 生成 `tauri-desktop-1280x800.png` |
| `src/game/host-overlays.ts` 与 `src/game/overlay/` 的宿主覆盖层骨架，以及 `src/game/remake-notes/` 的「復刻說明」内容：三个入口按钮与「畫面縮放」同行右对齐、各自固定开在第一个分页；三个覆盖层共用并持久化「繁體／简体」文件语言、同步转换正文与辅助标签且不重设当前分页／图鉴选中项；说明的五个分页（含键盘／鼠标／手柄操作说明，以及免責聲明的权利归属、非官方／非商业边界与宽窄屏排版）、按键不得抵达战场、`Esc` 关闭并交还焦点、面板打开时敌方阶段照常推进 | 无独立单元测试（策展文字为常量，覆盖层需要 DOM） | `remake-notes.spec.ts` |
...
| `styles.css` 的战场层序（底板 → 画布 → `A/0000` 边框与 `A/0005` 雕像前景）：「平滑」缩放会把相机视口 `(40,23,400,308)` 的硬透明边缘向外晕开，边框必须最后绘制 | 无独立单元测试（纯合成顺序） | `display-scaling.spec.ts` 的「the battlefield never bleeds through the window frame」，像素解码见 `screenshot-pixels.ts` |
| `src/game/chrome-composite.ts` 的三张边框合成层（战场画框含雕像前景、右栏单位资料框、右栏战术面板框）：图磚按原版落点 1:1 画进同一张画布，非整数倍缩放才不会在 y=23/57/137/331、x=40/440 与右栏 y=149 分隔缝上抗锯齿出裂纹；`.unit-detail-shade` 的下缘也因此要藏进框图整列不透明的地方 | 无独立单元测试（需要 canvas 2D 与已解码的原版 PNG） | `display-scaling.spec.ts` 的「the window frame shows no seam where its tiles meet」：伪造 Tauri 执行阶段取得 1.59375 倍非整数缩放，垂直接缝量亮度、左侧边框栏用底板换色差分并带正对照、右栏分隔缝用底色换色判「纯不纯」；量测前必须藏掉盖在右栏上的 `.debug-toolbar`。合成内容与右栏叠放次序见 `stage0.spec.ts` 的画框像素断言 |
| 模块 23 啟動流程：加载页先完成开场 PNG 与 `MUSIC/14`／`MUSIC/1` 解码，玩家手势解锁 Web Audio 后才起动 Softstar Logo；其后为加法式调色板淡入、開場背景淡出淡入与 `A/23`+`A/24` 点阵滚动文字（行按 `DS:07C0`/`DS:07CA` 两条遮挡条逐扫描线揭开与遮住、左起点 `x=160`、`REMAKE-113` 的整体下移 8 像素）、標題 8×8 抖动溶解、点阵菜单文字与 50% 棋盘高亮、两套素材与 1608 tick 空闲重播、`Esc`／鼠标右键共用的取消语义与 `REMAKE-114`／`REMAKE-115` 的 `MUSIC/1` 只播一次、取消不打断；全部 Canvas／菜单 PNG 解码后才建立时间原点并启动 RAF | `startup.test.ts`、`staged-render-asset-cache.test.ts` | `startup.spec.ts` 与 `resource-loading.spec.ts` 的音乐准备／手势激活、开场解码冻结、精确单请求及截图；各关卡通过 `startup-controls.ts` 复用真实跳過开场路径——Logo 期间的按键只缩短停留，必须等滚动開場接管后再送一次动作 |
...
| 第 26 个可玩关卡（内部第 27 关）11–31 人部署、名单仅妮雅＋二十八候选、七名 NPC／三工兵名单外固定、槽 22 stableRemake 姓名“愛莉歐拉”／巨斧戰士通用肖像、SAY 51/52、五名叛军、四段妮雅线性到达区、无增援、当前 v96/v95/v94/v93/v92 显示身份迁移与 stage 28 路由 | `stage27-content.test.ts`、`stage27-battle.test.ts`、`deployment.test.ts`、`deployment-session.test.ts`、`save.test.ts`、`stage-runtime.test.ts` | `stage27.spec.ts` |
...
| 第 36 个可玩关卡（内部第 37 关）二十七人部署对究極女神三部位、SAY 81、最高难度数值、回復轮原版顺序／冰雪轮頭最后、头部／共享手部术法交替、棋盘顺序随机目标、碧娜維姬 `D/8` 部位肖像、冰雪／混乱免疫、毒写入与每轮三分之一、攻防下降写入、禁咒写入但不封专属行动、九字段隐藏、全灭目标、五通道无增援、v96/v95/v94/v93 与历史 stage 49 边界迁移 | `stage37-content.test.ts`、`stage37-battle.test.ts`、`technique-lab.test.ts`、`actions.test.ts`、`save.test.ts`、`stage-runtime.test.ts`、`debug-roster-profiles.test.ts`、`construction-terrain-assets.test.ts` | `stage37.spec.ts`、`technique-lab.spec.ts`、`stage36.spec.ts`、`debug.spec.ts` |
...
| `controller.ts` 的 `groupCommandSpeaker`：`REMAKE-122` 让 `全部休息`／`自由行動` 由本关主将发出（玩家军团 `commanderId`，没有登记的 `stage-00/01/02/21/22` 回退到妮雅槽 `1:0`），`跟隨主將` 仍由玩家刚指定的临时主将发出；信息栏文字与上窗肖像、姓名牌共用同一个说话者 | 无独立单元测试（`GameController` 需要 `localStorage` 与 DOM） | `stage0.spec.ts` 的 `REMAKE-122`；显式军团主将的关卡见 `stage3.spec.ts` 的希蜜命令对白 |
| `dialogue-window-animation.ts`、`ui.ts` 的逐字前置条件与 `A/18` 对话窗开闔动画：`WU`／`WD` 的 11 步横向展开（80→400 px）、`CU`／`CD` 的 12 步收回、展开播完前一个字都不出现（同一扇窗接着放下一页时不重播展开，逐字同步开始）、展开途中的主操作先跳完展开再兑现成「补完本页」而不是白按一次、收合一开始就先隐藏窗内正文、收合期间对话层与肖像不得提前消失、`data-dialogue-closing` 残影不再接受点击 | 无独立模拟数值测试（Vitest 跑在 node 环境，无 DOM 与 Web Animations） | `dialogue-window-animation.spec.ts`；判定「对话是否仍在进行」统一走 `dialogue-controls.ts` 的 `activeDialogueRecord` |
...
| 模块 25 过场底纹：`A/20` 以 `40×40` 为重复单元铺满 `640×350`（全部关卡共用一张）、`BK/<id>` 画在原生 `(160,80)`，以及模块 29 战场内 `PP` 不得长出底纹 | 无独立模拟数值测试；证据在 `story-presentations.json#storyBackdrop`，运行时图块由 `scripts/generate-portrait-catalog.mjs` 裁出并断言 | `story-backdrop.spec.ts`；各关卡核对 `BK/<id>` 底图统一走 `story-background.ts` 的 `expectStoryBackground`——资源包上线后 `background-image` 只解析成 `blob:`，档名改由 `ui.ts` 一并写下的 `--story-illustration-source` 核对 |
| `audio.ts`、`audio-settings.ts`、`sound-effect-transport.ts` 的常驻 AudioBuffer／低延迟 Web Audio 调度、独立五档总音效增益、四类音效请求门与走路声 `E/14`：玩家／我方自动／工兵構築／半龍戰士傳送／逐关脚本移动各请求一次，返悔与 `REMAKE-106` 的敌方阶段静音，走路声随移动演出结束以 GainNode 淡出收尾（`data-walk-effect-active`） | `sound-effect-cache.test.ts`、`audio.test.ts` 的增益与通道路由、`preferences.test.ts` 的旧偏好迁移 | `resource-loading.spec.ts` 的全景／地图技能预载请求类型，`combat-lab.spec.ts` 与 `technique-lab.spec.ts` 的正式映射、零 `media` 请求和 AudioContext 激活；音量门与走路见 `stage0.spec.ts` 的 `RHP-05`／`RHP-05b` 与 `S00-A`，我方自动见 `stage2.spec.ts`、敌方静音见 `stage3.spec.ts` 的 `S03-N/O` |
