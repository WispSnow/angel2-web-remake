...
| 第 33 个可玩关卡（内部第 34 关）直接 1–11 人部署、SAY 66、芙瑪羅妮／蕾娜吉芙与十七名静态守军、十九名共享专家追击、全灭目标、五通道无动态增援、无胜利 SAY、v63/v62 与冻结 stage 35 路由 | `stage34-content.test.ts`、`stage34-battle.test.ts`、`save.test.ts`、`stage-runtime.test.ts`、`debug-roster-profiles.test.ts`、`construction-terrain-assets.test.ts` | `stage34.spec.ts`、`stage33.spec.ts`、`debug.spec.ts` |
