...
test("S27-C–E: opening story preserves the mixed 31-allied-unit force and five rebels", async ({ page }) => {
  ...
  await expect(page.getByTestId("objective-panel-text"))
    .toHaveText(NATIVE_OBJECTIVE_PANEL_TEXT[27].join("\n"));
  await captureVisualAudit(page.getByTestId("game-screen"), {
...
test("S27-I/J: victory story saves and enters the stage-28 prebattle", async ({ page }) => {
  ...
  expect(await state(page)).toMatchObject({
    ...
    consumedEventIds: ["stage-28-prebattle-story"],
  });
});
