...
test("S33-H/I: ordinary victory saves v62 and reaches the frozen stage-34 boundary", async ({ page }) => {
  ...
  await expect(page.getByText(/stage-34/u)).toBeVisible();
  await captureVisualAudit(page.getByTestId("game-screen"), {
    path: `${ARTIFACT_DIR}/stage33-stage34-boundary.png`,
  });
});
