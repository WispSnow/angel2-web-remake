...
const STAGES: StageUnderTest[] = [
  ...
  },
  {
    nativeStage: 18,
    load: async () => {
      const m = await import("../../src/game/content/stage18");
      return { sources: m.STAGE18_SOURCES, objective: m.STAGE18_DEFINITION.objective, enemyUnits: m.STAGE18_SEMANTIC_ENEMY_UNITS };
    },
  },
  {
    nativeStage: 19,
    load: async () => {
      const m = await import("../../src/game/content/stage19");
      return { sources: m.STAGE19_SOURCES, objective: m.STAGE19_DEFINITION.objective, enemyUnits: m.STAGE19_SEMANTIC_ENEMY_UNITS };
    },
  },
];
...
describe("per-stage victory-condition records (REMAKE-051)", () => {
  it("resolves the module-29 objective table to the machine-confirmed records", () => {
    expect(STAGES.map(({ nativeStage }) => recordFor(objectiveRecordTable, nativeStage))).toEqual([
      98, 172, 106, 107, 108, 160, 109, 162, 110, 111, 113, 114, 159, 83, 84, 85, 86, 87, 88,
    ]);
