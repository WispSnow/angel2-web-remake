...
describe("stage 2 generated content", () => {
  ...
  it("decodes all terrain cells and preserves the complete fixed roster", () => {
    ...
    expect(STAGE2_SEMANTIC_ALLIED_UNITS.map(({ slot, aiBehavior, position, initialClassId }) => ({
      ...
      { slot: 51, aiBehavior: 11, position: { x: 20, y: 33 }, initialClassId: undefined },
      { slot: 52, aiBehavior: 11, position: { x: 23, y: 33 }, initialClassId: undefined },
      { slot: 53, aiBehavior: 11, position: { x: 27, y: 33 }, initialClassId: undefined },
      { slot: 54, aiBehavior: 11, position: { x: 29, y: 33 }, initialClassId: undefined },
