...
describe("original engineer construction tiles", () => {
  ...
  it("keeps the stage 28 pair byte-identical when both native tokens are 16", async () => {
    const [ironPlate, obstacle] = await Promise.all(
      kinds.map((kind) => readFile(tileFile(kind, "stage-28"))),
    );
    expect(ironPlate.equals(obstacle)).toBe(true);
  });

  it("keeps the stage 29 pair byte-identical when both native tokens are 16", async () => {
    const [ironPlate, obstacle] = await Promise.all(
      kinds.map((kind) => readFile(tileFile(kind, "stage-29"))),
    );
    expect(ironPlate.equals(obstacle)).toBe(true);
  });

  it("keeps the stage 30 pair byte-identical when both construction tokens are absent", async () => {
    const [ironPlate, obstacle] = await Promise.all(
      kinds.map((kind) => readFile(tileFile(kind, "stage-30"))),
    );
    expect(ironPlate.equals(obstacle)).toBe(true);
  });

  it("keeps the stage 31 pair byte-identical when both native tokens are 77", async () => {
    const [ironPlate, obstacle] = await Promise.all(
      kinds.map((kind) => readFile(tileFile(kind, "stage-31"))),
    );
    expect(ironPlate.equals(obstacle)).toBe(true);
  });

  it("keeps the stage 32 pair distinct for native tokens 3 and 101", async () => {
    const [ironPlate, obstacle] = await Promise.all(
      kinds.map((kind) => readFile(tileFile(kind, "stage-32"))),
    );
    expect(ironPlate.equals(obstacle)).toBe(false);
  });

  it("keeps the stage 33 pair byte-identical when both native tokens are 2", async () => {
    const [ironPlate, obstacle] = await Promise.all(
      kinds.map((kind) => readFile(tileFile(kind, "stage-33"))),
    );
    expect(ironPlate.equals(obstacle)).toBe(true);
  });

  it("keeps the stage 34 pair byte-identical when both construction tokens are absent", async () => {
    const [ironPlate, obstacle] = await Promise.all(
      kinds.map((kind) => readFile(tileFile(kind, "stage-34"))),
    );
    expect(ironPlate.equals(obstacle)).toBe(true);
  });
  ...
  it("keeps the stage 36 pair byte-identical when both native tokens are 23", async () => {
    const [ironPlate, obstacle] = await Promise.all(
      kinds.map((kind) => readFile(tileFile(kind, "stage-36"))),
    );
    expect(ironPlate.equals(obstacle)).toBe(true);
  });

  it("keeps the stage 37 pair distinct for native tokens 78 and 23", async () => {
    const [ironPlate, obstacle] = await Promise.all(
      kinds.map((kind) => readFile(tileFile(kind, "stage-37"))),
    );
    expect(ironPlate.equals(obstacle)).toBe(false);
  });
});
