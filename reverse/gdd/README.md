...
当前 GDD 为 **Draft 0.47 / 待用户评审**。28 个原始文件与 14 个索引容器已完整盘点，11,168 个字形和主要图像/VOC/RIX/剧情记录均已提取；10 个运行模块也已完成 LH7 与 LZEXE 两层解包。39 个兵种、31 条转职边、普通战斗、射击、全部玩家技术、八槽状态、AI 主链、23 个地形规则槽、全部可达范围模式、39 个普通模板与五个特殊入口均有机器规格。六个 `.TST`、部署与普通单场生命周期、84 项目标、38/38 个逐关处理器、标题/剧情/反馈/HUD/战后/终幕编排、`WD` 和第 26 关阶段尾特效同样闭合。Draft 0.46 以模块 21/25/29 的原生 VGA Map Mask 证明四条资源流应映射到颜色 bit `3..0`，全量重渲染受影响 PNG，并恢复模块 21 的 password 专用调色板；Draft 0.47 又闭合骑士／战士十一种末端职业的专属分支和飛龍玩家攻击后移动。机器审计现覆盖 171 条证据：150 条纯 C、21 条混合边界、实现必需未知 0；第一阶段只差用户评审，全战役仍冻结于已有有界例外之外。
...
- `phase-1-review-checklist.md`：Draft 0.47 的用户评审摘要、保留边界与签署项。
...
- `../notes/campaign-roster-and-stage0.md`：双阵营角色显示描述符、职业视觉回退、新游戏初值、
  第 0 关阵容／开场镜头与第 5 关双首领身份。
