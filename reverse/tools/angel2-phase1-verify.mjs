...
async function verifyPhase1Artifacts(root) {
  ...
  assert(evidence.rows === 151 && evidence.confirmedRows === 130 && evidence.mixedRows === 21,
    "phase-1 evidence counts must remain 151/130/21");
  ...
  assert(gdd.includes("版本：Draft 0.56"), "GDD is not Draft 0.56");
  assert(gdd.includes("151 项：130 项纯 C、21 项混合边界、实现必需未知 0"),
