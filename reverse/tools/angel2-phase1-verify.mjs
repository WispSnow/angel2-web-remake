...
async function verifyPhase1Artifacts(root) {
  ...
  assert(evidence.rows === 167 && evidence.confirmedRows === 146 && evidence.mixedRows === 21,
    "phase-1 evidence counts must remain 167/146/21");
  ...
  assert(gdd.includes("版本：Draft 0.73"), "GDD is not Draft 0.73");
  assert(gdd.includes("167 项：146 项纯 C、21 项混合边界、实现必需未知 0"),
