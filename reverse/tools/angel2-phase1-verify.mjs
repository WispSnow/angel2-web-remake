...
async function verifyPhase1Artifacts(root) {
  ...
  assert(evidence.rows === 155 && evidence.confirmedRows === 134 && evidence.mixedRows === 21,
    "phase-1 evidence counts must remain 155/134/21");
  ...
  assert(gdd.includes("版本：Draft 0.60"), "GDD is not Draft 0.60");
  assert(gdd.includes("155 项：134 项纯 C、21 项混合边界、实现必需未知 0"),
