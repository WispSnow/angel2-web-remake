...
async function verifyPhase1Artifacts(root) {
  ...
  assert(evidence.rows === 164 && evidence.confirmedRows === 143 && evidence.mixedRows === 21,
    "phase-1 evidence counts must remain 164/143/21");
  ...
  assert(gdd.includes("版本：Draft 0.70"), "GDD is not Draft 0.70");
  assert(gdd.includes("164 项：143 项纯 C、21 项混合边界、实现必需未知 0"),
