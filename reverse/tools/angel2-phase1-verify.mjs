...
async function verifyPhase1Artifacts(root) {
  ...
  assert(evidence.rows === 171 && evidence.confirmedRows === 150 && evidence.mixedRows === 21,
    "phase-1 evidence counts must remain 171/150/21");
  ...
  assert(gdd.includes("版本：Draft 0.77"), "GDD is not Draft 0.77");
  assert(gdd.includes("171 项：150 项纯 C、21 项混合边界、实现必需未知 0"),
