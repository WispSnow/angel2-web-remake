...
async function verifyPhase1Artifacts(root) {
  ...
  assert(evidence.rows === 170 && evidence.confirmedRows === 149 && evidence.mixedRows === 21,
    "phase-1 evidence counts must remain 170/149/21");
  ...
  assert(gdd.includes("版本：Draft 0.76"), "GDD is not Draft 0.76");
  assert(gdd.includes("170 项：149 项纯 C、21 项混合边界、实现必需未知 0"),
