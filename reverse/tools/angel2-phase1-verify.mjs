...
async function verifyPhase1Artifacts(root) {
  ...
  assert(evidence.rows === 160 && evidence.confirmedRows === 139 && evidence.mixedRows === 21,
    "phase-1 evidence counts must remain 160/139/21");
  ...
  assert(gdd.includes("版本：Draft 0.65"), "GDD is not Draft 0.65");
  assert(gdd.includes("160 项：139 项纯 C、21 项混合边界、实现必需未知 0"),
