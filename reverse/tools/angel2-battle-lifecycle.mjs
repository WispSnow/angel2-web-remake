...
function summarizeDeployment(templates, templatesPath) {
  ...
  const stages = templates.stages.map((stage) => {
    ...
    return {
      ...
      fixedRosterUnits: deployment.fixedRosterUnitSlots.length,
      fixedBoardOnlyUnits: deployment.fixedBoardOnlyUnitSlots.length,
      ...
      fixedRosterUnitSlots: deployment.fixedRosterUnitSlots,
      fixedBoardOnlyUnitSlots: deployment.fixedBoardOnlyUnitSlots,
...
async function extract(module27Path, module29Path, templatesPath, outputPath) {
  ...
  const output = {
    ...
    deployment: {
      ...
      fixedRule: "a unit already occupying a side-1 cell is selected and fixed for battle; it appears in the roster only when its scenarioUnitFlag is nonzero, otherwise it remains board-only",
