...
逐单位行动之后还有一层检查（2026-09-02 按线性地址扫描全部分发器调用点确认，共六处：
`0000:49D6/49E3`、`0000:4A57/4A61`、`1000:164F/1659`）：side 1 自动阶段 `1000:1542` 与
side 2 AI `1000:14A6` 共用调度器 `1000:1595`，其逐单位钩子 `1000:15F4` 在每个单位行动之后
先后调用失败、胜利分发器，任一返回 `Y` 就置 DS:`1086h` 提前结束当前阶段；但 999 只由主循环
`0000:4A6E` 在上述第 1／6 步的检查后写入。因此自动引路者（第 4 关葛蒂拉斯、第 9 关多莉）在
自动阶段达成的到达，原版要到随后的敌方阶段才结算：第一个被调度的敌人行动一次，钩子停止
阶段，主循环再先查失败后查胜利。玩家自己操作的到达者在行动提交后回到第 1 步即判定。
...
原版这条路由只重建战场模板，不回滚上层战役角色状态。战斗中已经写入战役职业／累计
经验的变化会成为模块 27 再次生成同关 `JUST` 的输入；主动撤退走相同的
`module 27 / currentStage` 路由。因此失败或撤退后可以保留本次尝试取得的经验、升级和
转职，再反复进入同一关继续成长。该可见结果由用户于 2026-08-04 在原版流程中确认；
控制流边界由模块 29 的失败／撤退出站、模块 27 的战役职业导入和 `ME_DATA/ME_EXP`
战役数组共同支持。
