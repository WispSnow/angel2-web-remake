...
新战分支在开始首回合前还执行难度属性初始化。除场景 3、8、11 外，`0000:4ABE` 重复 `LV_HARD+1` 次调用 `0000:538B`；每次由 `0000:539B` 把 side 2 活动单位经验写为下一累计阈值加 1。随后 `0000:536B/537B` 通过 `540D` 重建双方并回满生命，`LV_HARD==3` 的 side 2 加半发生在重建链末端。编号 `WAR` 恢复不走这条播种链，因而读档不能再次升级或加成。
