...
菜单画面也已闭合。`045Bh` 在保留战场画面的另一 VGA 页先调用 `0794h` 画固定底框，再由
`0693h` 按原生候选顺序画职业图；它没有标题、职业名或属性文字。底框主体为
`(160,110) 316×110` 的调色板色 1 矩形，后方有 `(176,126) 316×110` 的黑色阴影；上下边
分别把 `A/0006` frame 1／0 以 8 px 步长重复 39 次，四角使用 frame 5，左右五条竖线的
调色板序列分别为 `14/0/11/14/0` 与 `0/14/11/0/14`。包含阴影后的固化范围为
`(160,110) 332×126`。

职业图直接取 `A/0002`：首张在 `(184,134)`，后续每项向右 56 px，原生图形为
`40×43`。每个可见框从 `(180,130)` 起为 `48×52`，黑色外线、白色右／下线，内部普通色
为调色板 1；`03A0h` 在移动选择时把新项内部改为调色板 12（绿色），旧项恢复色 1。原版
因此是单行纯图像选择，不暗化战场，也不显示复刻版曾采用的 2×2 属性卡。代码签名、矩形、
五个静态热区模板、资源加载指针 `A/0002→DS:0219h`／`A/0006→DS:02A5h` 已固化到
`promotion-table.json.runtimeEvidence.promotionMenuPresentation`；运行时合成资产只消费这份
合同和已验证的原版调色板／图帧。
