...
- 已生成 8,725 张位平面 PNG。`B.SWF` 的 44 个偶数记录现按原生寻址确认为每组 128 张 `40×44` 图块，并已生成 44 张 `50×50` 完整战场图、44 张 `150×150` 地形小地图、44 张地形+占用叠加图，以及五张含真实视口框的编号存档小地图；旧的 `32×16` 假设已废弃。
...
- 密码/图册复制保护已由 16 段 GO/模块 21/模块 29/`PLAY.COM` 签名闭合：首次模块 29 出站经 `MIMA_PASS` 改走模块 21、三次回答写向量 0/1/3、`C/0032` 的 28 项坐标/答案码、蓝/绿/橙/棕/紫/白六个可见按钮与内部码 `2/1/0/3/4/5`、重输/失败锁死、八个直接资源及启动器固定挑战 16→答案码 2（默认蓝色）均已结构化；提示前 DOS 向量检查只保留为兼容性取证。
...
| `C.SWF` | 60 | 34 | 170 | 0 | 81 | 0 | 0 | 图像包；`C/33` 已绑定为 22 帧制作人员姓名，模块 33 另从 `C/0..30` 随机选择战绩卡装饰图 |
...
| `MUSIC.SWF` | 60 | 50 | 0 | 0 | 0 | 0 | 41 | 主音乐容器，另有 9 条空占位；`MUSIC/1` 已绑定标题，`MUSIC/14` 已绑定滚动开场，并闭合各自 RIX 起止点 |
...
| 调色板映射 | 已恢复至少两套原生 16 色 VGA DAC 表 | 将每个资源加载调用与传入的调色板指针关联 |
