...
铁板与障碍同样没有独立图形记录、VOC、描述符动画或等待。外层入口把选中线性格号直接交给地形写入器：

- `1K` 从逐关原始地图 `(16,25)` 复制源 token；
- `2K` 从 `(16,26)` 复制源 token；
- 从目标格起向线性地址 `+0..+4` 连续写 `sourceToken..sourceToken+4`。

地形写入就是规则结算点；动作返回后的常规棋盘重绘才让新地形可见。Web 复刻若添加施工动画，只能作为增强/Mod 表现，不能标作原版资源还原。
