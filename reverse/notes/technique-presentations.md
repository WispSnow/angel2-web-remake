...
同一次复核发现电波循环 `1000:6E46` 的扫描带判定可能永远不成立，需要 DOSBox 实机确认后
才能改动实现，暂不据此修改表现：
...
同一判定在 `0000:65A5`（写 `DS:58A5h` 地图特效 sprite 层的回调）里写法正确：
`xor ax,ax; mov al,bl; sub ax,[0x522c]; cmp ax,[0x5228]; ja`。若 `1000:6E46` 确实恒跳过，
则原版电波并非“在扫描带敌方占用格绘制 `MAGIC/31/24/25/26` 的 `5/6`、`6/7`、`5/6`、
`12/13` 单格描述符”，而是由 `0000:6599 → 0000:97DC → 0000:65A5` 对可见 `10×7` 窗口逐格
写入“范围值−阈值”作为 sprite 码——不分阵营、不分占用，且帧号随距离渐变。各级波宽
`9/5/4/11` 与资源帧数 `12/7/6/13` 恰好相容，与该假设一致。

固定等待（44/32/28/60 tick）由 `0000:64A1` 提供，与是否真的绘制无关，所以时间线结论不受
影响。判定前不要改动电波实现。
