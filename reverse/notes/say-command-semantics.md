...
视觉资源方面，`D/0..67` 的 `112×112` 主肖像已全部按 DS:`0DE6` 出图；此前通用 renderer 因 D/63 的 mask 指针表非单调而漏掉记录 63，新提取器按原生“每平面独立读当前 image pointer”规则恢复了它。`A/20` 同样由原生单指针规则恢复为 `40×41` 清理纹理。
