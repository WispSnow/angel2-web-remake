...
const storyBackgroundPath = path.join(publicAssetPath, "story-stage23-background.svg");
const inputPaths = {
  ...
  prebattleStory: reversePath("parsed/dialogue/0045.json"),
...
const storyPages = {
  "stage-23-prebattle-story": compileNativeStory(
    parseInput("prebattleStory"), 45, portraitSpeakers, { includeBackground: true },
  ),
...
identityHash.update("stableRemake\0REMAKE-058\0");
...
// SAY/45 uses a framebuffer-backed prebattle presentation. Preserve the stage
// context with a deterministic crop of the confirmed stage-23 map instead of
// falling through to the unrelated palace background.
const storyBackground = `<svg xmlns="http://www.w3.org/2000/svg" width="320" height="200" viewBox="800 1332 640 400">
...
console.log(`wrote stage 23 maps, story background, and music with identity ${contentIdentity}`);
