...
const CONTEXTUAL_LINES = [
  ...
  // `1000:1C93`/`1CFB`: a follower that found its route to the leader, spoken
  // from its own cell before it walks the route.
  { key: "rallyingToGeneral", selector: "03h", record: "rallying-to-general", gate: "aiDialogue", text: "將軍我來了." },
...
const randomGate = contextualTable.randomGate;
if (!randomGate || !Number.isInteger(randomGate.modulus) || !Number.isInteger(randomGate.playsWhenAtMost)
  || randomGate.playsWhenAtMost >= randomGate.modulus) {
  throw new Error("missing native contextual-line coin evidence");
}
...
const contextualLines = Object.fromEntries(CONTEXTUAL_LINES.map((line) => {
  ...
  if (typeof entry.randomGate !== "boolean"
    || entry.randomGate === randomGate.exemptSelectors.includes(entry.selector)) {
    throw new Error(`contextual line ${line.selector}: coin flag disagrees with 0000:C981`);
  }
  return [line.key, {
    record: line.record,
...
const source = `// Generated by scripts/generate-ai-technique-dialogue.mjs from native AI evidence.\n`
  ...
  + ` */\n`
  + `export const NATIVE_CONTEXTUAL_LINE_RANDOM_GATE = ${JSON.stringify({
    entry: randomGate.entry,
    coin: randomGate.coin,
    accumulator: randomGate.accumulator,
    modulus: randomGate.modulus,
    playsWhenAtMost: randomGate.playsWhenAtMost,
    exemptSelectors: randomGate.exemptSelectors.map((selector) => Number.parseInt(selector, 16)),
  }, null, 2)} as const;\n\n`
  + `/**\n`
