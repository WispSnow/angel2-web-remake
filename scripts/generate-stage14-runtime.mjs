...
if (!originalObjectiveText.includes("把女帝「維絲塔」打醒") || !originalObjectiveText.includes("「妮雅」戰敗")) {
...
await Promise.all([
  ...
  copyFile(inputPaths.playerEntryMusic, path.join(publicAssetPath, "battle-stage14-player-entry.wav")),
  copyFile(inputPaths.playerLoopMusic, path.join(publicAssetPath, "battle-stage14-player-loop.wav")),
  copyFile(inputPaths.enemyEntryMusic, path.join(publicAssetPath, "battle-stage14-enemy-entry.wav")),
  copyFile(inputPaths.enemyLoopMusic, path.join(publicAssetPath, "battle-stage14-enemy-loop.wav")),
]);
