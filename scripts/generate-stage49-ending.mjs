...
// These C records are byte-identical to the already published full-combat
// backdrops. Keep one runtime path while retaining the native C render as the
// evidence source for the equality check below.
const SHARED_DECORATION_RECORDS = [0, 5, 6, 7, 8, 11, 12, 14, 15, 16, 18, 19, 20, 21, 22, 23, 24, 25, 28, 29];
...
const generated = `// Generated by scripts/generate-stage49-ending.mjs from native postgame evidence.\n`
  ...
  + `export const STAGE49_SHARED_DECORATION_RECORDS = ${json(SHARED_DECORATION_RECORDS)} as const;\n`
...
for (let record = 0; record <= 30; record += 1) {
  const name = String(record).padStart(2, "0");
  const source = reversePath("renders/planar/C", String(record).padStart(4, "0"), "00.png");
  const target = path.join(publicRoot, "decorations", `${name}.png`);
  if (SHARED_DECORATION_RECORDS.includes(record)) {
    ...
    copies.push(copyFile(source, target));
  }
}
