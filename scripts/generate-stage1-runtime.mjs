...
if (!objectiveSourceText.some((text) => text.includes("打敗敵人首領「芳」"))
  || !objectiveSourceText.some((text) => text.includes("「妮雅」戰敗"))) {
  throw new Error("SAY/0083 no longer contains the stage 1 objective text");
}
...
function compileStory(document, record) {
  ...
  for (const action of document.actions) {
    if (action.op === "show_portrait") {
      ...
      if (!activeSlot) throw new Error(`SAY/${String(record).padStart(4, "0")} has a line break without an active window`);
