...
const inputPaths = {
  ...
  title: reversePath("parsed/dialogue/0161.json"),
...
// The bottom stage label comes from module 29 `0000:4F41`, which scans `DS:30BA`
// for the current stage and draws the first entry's record. Stage 38 is listed
// twice: SAY/0161 first, then a record-0 entry the scan never reaches. The Web
// version once shipped its own "異世界" here, so lock the name to that entry.
const titleRecordEntry = parse("storyPresentations")
  .globalReachabilityAudit.tables.postBattle.entries.find(({ key }) => key === 38);
if (titleRecordEntry?.dialogueRecord !== 161 || path.basename(inputPaths.title) !== "0161.json") {
  throw new Error(`stage 38 title record changed: ${JSON.stringify(titleRecordEntry)}`);
}
const titleText = parse("title").actions.filter(({ op }) => op === "text")
  .map(({ text }) => text).join("").replace(/[\t$]/gu, "").trim();
if (titleText !== "瑪姬的墓園") throw new Error(`stage 38 title changed: ${titleText}`);
...
const generatedSource = `// Generated by scripts/generate-stage38-runtime.mjs from stage 38 machine evidence.\n// Do not hand-edit: regenerate after the evidence pipeline changes.\nimport type { DialoguePage } from "../types";\n\n`
  ...
  + `export const STAGE38_TITLE = ${json(titleText)};\n`
