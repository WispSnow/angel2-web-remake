...
/**
 * Every intro row starts at the same left margin: 0000:1242 writes 00A0h to
 * DS:510E before each row draw, exactly as 0000:1AD1 writes 0208h for the menu
 * labels. The rows look centred because each Big5 line carries its own leading
 * ideographic spaces, so measuring the padded string and centring it as well
 * would apply that indent twice — see `reverse/notes/title-presentations.md`.
 */
function nativeRow(text, originX) {
  const { glyphs, width } = layout(text, originX);
  assert.ok(originX + width <= SCREEN.width, `intro row overflows the screen: ${text}`);
  return glyphs;
}
...
// 0000:2CCE draws the outline mask at (x, y) and (x+2, y) before the cell itself
// at (x+1, y+1), so a menu label stays readable over the highlight stipple and
// every string sits one pixel down and right of its cursor.
const glyphText = presentations.glyphText;
assert.equal(glyphText.glyph.rows, GLYPH_HEIGHT);
assert.equal(glyphText.outline.rows, GLYPH_HEIGHT + 2);
assert.deepEqual(glyphText.outlineDilationRowOffsets, [0, 1, 2]);
assert.deepEqual(glyphText.passes.map(({ shape, dx, dy }) => [shape, dx, dy]),
  [["outline", 0, 0], ["outline", 2, 0], ["glyph", 1, 1]]);
assert.equal(glyphText.colorIndex, 15);
assert.equal(glyphText.outlineColorIndex, 0);
...
// The two colour-0 bars 0000:11DA paints after the rows are what makes a line
// uncover and cover one scanline at a time instead of appearing whole, so the
// runtime needs the window they leave open, not just the row's own Y range.
assert.deepEqual(intro.draw.visibleWindow, { x: 160, y: 273, width: 400, height: 44 });
assert.equal(intro.draw.upperMask.colorIndex, 0);
assert.equal(intro.draw.lowerMask.colorIndex, 0);
assert.equal(intro.draw.textOriginX, 160);
...
const introLines = intro.scroll.assignments.map(({ update, slot, entryIndex }) => {
  ...
  return { update, slot, text, glyphs: text === "" ? [] : nativeRow(text, intro.draw.textOriginX) };
});
...
const generated = `// Generated by scripts/generate-startup-runtime.mjs from module-23 evidence.\n`
  ...
  + `/** 0000:2CCE: outline at (x,y) and (x+2,y), then the cell at (x+1,y+1). */\n`
  + `export const STARTUP_TEXT = ${json({
    color: cssColor(TITLE_PALETTE[glyphText.colorIndex]),
    outlineColor: cssColor(TITLE_PALETTE[glyphText.outlineColorIndex]),
    outlineRows: glyphText.outline.rows,
    dilationRowOffsets: glyphText.outlineDilationRowOffsets,
    outlinePasses: glyphText.passes
      .filter(({ shape }) => shape === "outline")
      .map(({ dx, dy }) => ({ dx, dy })),
    glyphOffset: (({ dx, dy }) => ({ dx, dy }))(
      glyphText.passes.find(({ shape }) => shape === "glyph")),
  })} as const;\n`
  ...
  + `export const STARTUP_INTRO = ${json({
    ...
    visibleWindow: intro.draw.visibleWindow,
