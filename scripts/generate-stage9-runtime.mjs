...
if (!originalObjectiveText.includes("「妮雅」到達死亡之谷的頂端")
  || !originalObjectiveText.includes("「妮雅」戰敗")) {
  throw new Error("SAY/0091 objective conflict changed");
}
  ...
  for (const action of document.actions) {
    if (action.op === "show_portrait") windows[action.slot].portrait = action.portraitId;
      ...
      for (const slot of ["upper", "lower"]) {
        ...
        page[slot] = {
          text: state.text,
          ...(state.portrait === undefined ? {} : { portrait: state.portrait, speaker: portraitSpeakers[state.portrait] }),
        };
      }
      pages.push(page);
    }
...
await Promise.all([
  ...
  copyFile(inputPaths.playerEntryMusic, path.join(publicAssetPath, "battle-stage9-player-entry.wav")),
  copyFile(inputPaths.playerLoopMusic, path.join(publicAssetPath, "battle-stage9-player-loop.wav")),
  copyFile(inputPaths.enemyEntryMusic, path.join(publicAssetPath, "battle-stage9-enemy-entry.wav")),
  copyFile(inputPaths.enemyLoopMusic, path.join(publicAssetPath, "battle-stage9-enemy-loop.wav")),
]);
