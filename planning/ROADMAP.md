...
| M14 | 第 8 关有界实现 | `implemented` | `M14-FB-01 / REMAKE-032` 补播、v23、迁移与视觉门禁通过，等待用户复验 |
...
胜利剧情接入、v23 与 v22 迁移、定向 Chromium、生产构建和完整文字截图审计，等待用户复验。
...
当前验收焦点是 M14 第 8 关 SAY 157 补播人工复验。第 9 关仍需独立纸面合同和用户授权；不得因第 8 关
实现完成而提前实现。
