...
| M01 | 第 1 关实施准备 | `specified` | P1–P6 纸面与接口方案完成，等待 M02 明确授权 |
...
2026-08-01 完成；它不会自动授权第 1 关代码，也不能借通用化提前实现新增射击、完整
状态生命周期或 `1F/1H/1L/1C` 之外的职业能力。
...
2. 建立与渲染器无关的部署模拟；
3. 复用 M00.6 的 `1F/1H`，新增 `1L/1C` 与敌方修女调度；
