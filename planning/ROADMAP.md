...
| M14.9 | 双方共享自动行动专家 AI | `accepted` | `REMAKE-037` 统一默认规划器并保留显式策略；`M14.9-FB-01 / REMAKE-060` 补完整通路、虚拟前线排队、等价安全攻击让路及 v47/v46 迁移 |
...
| M55 | 内部第 29 关有界实现 | `verified` | 证据生成、剧情、部署、全灭战、v57/v56/v55、七类调试场景与 stage 30 冻结路由通过；M55-FB-01 已修复愛莉歐拉出场后姓名丢失，等待用户复验 |
...
跟随当前职业。M54 为 `specified`；M55 已接入 v57/v56/v55、不提前播放 SAY 57 的 stage 30 冻结路由、
关前／部署／玩家回合／近胜／近败／胜利准备／完成路由七类调试场景，并按实际边界把初始镜头钳制为
`(36,23)`。定向自动与代表性视觉门禁均已通过，状态为 `verified`，等待用户普通入口复验。
...
第 0–24 关、内部第 26–28 关与既有非连续战斗均获用户接受；内部第 29 关已完成 M54 纸面
合同，M55 实现、姓名反馈修复与自动／视觉门禁已验证，等待用户普通入口复验。stage 30 及后续内容仍需独立纸面合同和用户授权，内部场景 25
不自行补造。
