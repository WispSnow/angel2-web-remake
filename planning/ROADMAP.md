...
| M67 | 内部第 35 关有界实现 | `verified` | 证据生成、固定棋盘、SAY 67/68、行为 12 待命、v64/v63、六类调试场景、两套成长档及 stage 36 冻结路由通过，等待用户试玩 |
...
12 无路线待命、全灭目标、五通道无动态增援、v64/v63 与 stage 36 冻结边界。证据生成内容、
六类调试场景、两套成长档与定向自动门禁已通过，M67 为 `verified`，等待用户普通入口试玩。
...
第 0–24 关、内部第 26–34 关与既有非连续战斗均获用户接受；内部第 35 关已完成 M66 纸面
合同和 M67 有界实现，自动／视觉门禁已验证，等待用户普通入口试玩。stage 36 及后续内容
