...
| M47 | 内部第 24 关有界实现 | `verified` | 证据生成、SAY 47/48、到达目标、v49/v48/v47、调试、直接 stage 26 路由及定向自动／视觉门禁通过，待用户试玩 |
...
守军、妮雅线性格号 `0..1030` 到达目标与胜利对白，并直接保存至 stage 26 冻结边界；
内部场景 25 不构造运行时。v49/v48/v47、调试、定向自动、生产构建与代表性视觉门禁已通过，
等待用户从普通入口试玩。
...
第 0–23 关与既有非连续战斗均获用户接受；内部第 24 关已获用户授权并进入有界实现。
stage 26 及后续内容仍需独立纸面合同和用户授权，内部场景 25 不自行补造。
