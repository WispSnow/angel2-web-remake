...
| M41 | 内部第 21 关有界实现 | `verified` | 非交互侦察过场、v43/v42、直接 stage 22 路由与定向自动／视觉门禁通过，待用户试玩 |
...
跳过普通胜利与存档提示、v43/v42 及 stage 22 冻结路由；定向自动与视觉门禁通过，等待
用户普通入口试玩。
...
第 0–20 关与既有非连续战斗均获用户接受；内部第 21 关已验证、等待用户试玩。stage 22
及后续内容仍需独立纸面合同和用户授权。
