...
| M61 | 内部第 32 关有界实现 | `verified` | 证据生成、直接部署、两段剧情、十八敌全灭、v61/v60、六类调试场景、两套成长档及 stage 33 冻结路由通过，等待用户试玩 |
...
v61/v60 与 stage 33 冻结边界。证据生成内容、六类调试场景、两套成长档、定向自动门禁
和代表性视觉审计已通过，M61 为 `verified`，等待用户普通入口试玩。
...
第 0–24 关、内部第 26–31 关与既有非连续战斗均获用户接受；内部第 32 关已完成 M60 纸面
合同和 M61 有界实现，自动／视觉门禁已验证，等待用户普通入口试玩。stage 33 及后续内容
