...
| M14 | 第 8 关有界实现 | `implemented` | `S08-A–K` 内容、固定战斗、保存、调试与视觉验收通过，等待用户试玩接受 |
...
目标、`REMAKE-031`、v22 与 stage 9 冻结边界均已闭合，无实施必需 `[TBD]`。

## M14：第 8 关有界实现

详细记录见 [`milestones/M14-stage-08-implementation.md`](milestones/M14-stage-08-implementation.md)。
用户接受 M12 并明确要求继续推进；本里程碑已按 `S08-A–K` 接入第 8 关，定向单元、Stage
7/8 Chromium、调试目录、生产构建和三背景／开战对白／战场／友军阶段视觉审计均已通过，
当前等待用户人工试玩接受。
...
当前验收焦点是 M14 第 8 关人工试玩。第 9 关仍需独立纸面合同和用户授权；不得因第 8 关
实现完成而提前实现。
