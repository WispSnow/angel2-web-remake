...
| M06 | 第 4 关有界实现 | `frozen` | 等待用户明确授权后生成内容、扩展特殊行动、存档、调试与普通入口 |
...
M05 状态为 `specified`，没有修改运行时。后续 M06 仍需用户明确授权；实施必须复用 M04.5
的唯一 manifest、生成内容、特殊行动目录、显式军团与存档分层，不能回到逐关控制器分支。
