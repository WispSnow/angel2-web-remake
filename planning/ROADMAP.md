...
| M06 | 第 4 关有界实现 | `implemented` | 内容、通用部署、三军团、结界脉冲、v15、调试和普通入口通过完整自动门禁与视觉审计，等待用户试玩接受 |
| M04+ | 后续能力切片 | `frozen` | 依次建立射击、状态、深层职业能力和特殊目标等系统 |
...
`route-pulse`、到达区胜利、v15、调试场景和普通入口。第 5 关仍只保留完成路由边界。
实现和完整自动门禁已于 2026-08-04 完成，当前只等待用户从普通入口完成第 4 关人工试玩；
在用户接受或给出有界反馈前，不进入第 5 关纸面合同或实现。

## 后续能力切片

用户于 2026-08-04 明确将后续顺序调整为“先逐个实现职业，再继续推进关卡”。独立
`/arena.html` 已作为这一阶段的组合验收表面：它允许在八类常用地形上配置当前已发布的
敌我职业并进入正式战斗，但不会自行解冻任何尚未闭合的职业规则。

每个职业仍按原战役证据与首次正常可达能力建立有界切片，建议顺序为：
...
每种能力先补系统规格、原版证据和竞技场验收实例，再解冻最小实现范围。第 5 关保持
冻结，直至用户确认职业实施阶段已满足推进条件；不要一次性建立未经证据和逐项验收的
全战役职业框架。
