...
| M14.9 | 双方共享自动行动专家 AI | `accepted` | `REMAKE-037/060/065/066/079/080/081/082/083` 统一双方规划器并补完整通路、法系落点、远程职责、单体预计残血集火、独立职业目标优先级及控制目标状态边界；系统基线 v70/v69，当前全局 v74 |
...
| M73 | stage 49 主线结局有界实现 | `accepted` | 故事、战绩、条件尾声、原版音画、75 槽战绩、累计存档次数、八个尾声直达调试夹具、v73/v72 与 stage 38 可见边界通过；用户确认 `STAGE-49 主線結局` 测试通过 |
...
`M14.9-FB-07 / REMAKE-082` 再把职业目标优先级从动作菜单中独立：十一法系最终 `+500`，
魔弓／弓／弩最终 `+300`，半龍戰士、巨龍騎士、工兵与其他职业为 0；菜单、AI 分派和
近战／远程职责不变。当前身份升级为 v69 / `expert-target-priority-ai-1`，v68 无损迁移。
`M14.9-FB-08 / REMAKE-083` 将混乱基础控制效用提高到 140，使同一目标威胁下比禁咒高
20；禁咒只为实际具有技术菜单的职业生成候选，已有同名状态与免疫继续直接剔除。本系统
身份升级为 v70 / `expert-control-targeting-ai-1`，v69 无损迁移；当前全局身份为 v74。
...
门禁与代表性视觉审计已通过；用户确认测试通过后，M69 转为 `accepted`。

M70/M71 记录见 [`milestones/M70-stage-37-specification.md`](milestones/M70-stage-37-specification.md)
与 [`milestones/M71-stage-37-implementation.md`](milestones/M71-stage-37-implementation.md)。第 36 个
可玩关卡按 B/0075、SAY `81/105/154` 与 `REMAKE-084/085` 闭合 1–27 人部署、究極女神
三个静态部位、回復轮原版顺序／冰雪轮頭最后、专属交替术法、碧娜維姬 `D/8` 部位肖像、
九字段隐藏、全灭目标、五通道无动态增援、冰雪／混乱／毒免疫、攻防下降写入与禁咒写入
但不封专属行动、v72/v71/v70 与 stage 49 主线结局入口。生成内容、七类调试场景、
两套成长档、定向自动门禁与代表性视觉审计已通过；用户确认测试通过后 M71 转为 `accepted`。
...
跨槽累计存档次数，完整播放
后停在隐藏 stage 38 可见边界。用户确认主线结局测试通过，M72/M73 转为 `accepted`。

M74/M75 记录见 [`milestones/M74-stage-38-specification.md`](milestones/M74-stage-38-specification.md)
与 [`milestones/M75-stage-38-implementation.md`](milestones/M75-stage-38-implementation.md)。
隐藏 stage 38“異世界”已接入 B/0077、18 格部署、44 名静态敌军、SAY/0164/0165、全灭／
妮雅失败优先与五通道无增援；完成后进入 `stage-39` 模块 46 终端。七页制作人员表按证据
连续八次自动转场，接入 `UN/55` 音乐并以 `UN/54` 永久 `The End` 循环。M75 已通过定向
规则、TypeScript、Chromium、调试中心、生产构建和视觉审计，等待用户普通入口试玩。
...
第 0–24 关、内部第 26–37 关与既有非连续战斗均获用户接受。stage 49 已完成 M72 纸面
合同和 M73 有界实现并获用户确认；隐藏 stage 38 已完成 M74 纸面合同和 M75 有界实现，
自动／视觉门禁已验证，等待用户普通入口试玩。其后没有新的战役关卡因本切片自动解冻。
