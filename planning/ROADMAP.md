...
| M14.9 | 双方共享自动行动专家 AI | `accepted` | `REMAKE-037/060/065/066/079/080/081/082/083` 统一双方规划器并补完整通路、法系落点、远程职责、单体预计残血集火、独立职业目标优先级及控制目标状态边界；系统基线 v70/v69，当前全局 v71 |
...
| M70 | 内部第 37 关纸面合同 | `specified` | B/0075、SAY 81、1–27 人部署、究極女神三部位、交替术法、九字段隐藏、全灭目标、五通道无增援、`REMAKE-084` 与 stage 49 结局边界闭合 |
| M71 | 内部第 37 关有界实现 | `verified` | 证据生成、直接部署、SAY 81、三部位最终首领战、v71/v70、六类调试场景、两套成长档及 stage 49 冻结边界通过，等待用户试玩 |
...
身份升级为 v70 / `expert-control-targeting-ai-1`，v69 无损迁移；当前全局身份为 v71。
...
可玩关卡按 B/0075、SAY `81/105/154` 与 `REMAKE-084` 闭合 1–27 人部署、究極女神
三个静态部位、固定行动顺序、专属交替术法、九字段隐藏、全灭目标、五通道无动态增援、
v71/v70 与 stage 49 主线结局冻结边界。生成内容、六类调试场景、两套成长档、定向自动
门禁与代表性视觉审计已通过，M71 为 `verified`，等待用户普通入口试玩。
...
第 0–24 关、内部第 26–36 关与既有非连续战斗均获用户接受；内部第 37 关已完成 M70 纸面
合同和 M71 有界实现，自动／视觉门禁已验证，等待用户普通入口试玩。stage 49 主线结局链、
隐藏 stage 38 及后续内容仍需独立纸面合同和用户授权，内部场景 25 不自行补造。
