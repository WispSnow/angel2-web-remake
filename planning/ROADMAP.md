...
| M12 | 第 7 关有界实现 | `implemented` | 第 7 关、普通胜利保存、v21、调试和定向自动／视觉验收完成，待人工接受 |
...
定向单元、Stage 6/7 Chromium、生产构建与双背景／部署／战场视觉审计均已通过，等待用户
人工试玩接受。
...
当前验收焦点是 M12 第 7 关人工试玩。第 8 关仍需独立纸面合同和用户授权；不得因第 7 关
实现完成而提前实现。
