...
const STAGE37_BATTLE_EVENT_IDS = [
  "stage-37-enter-deployment",
  "stage-37-opening-story",
] as const;

const STAGE37_COMPLETED_EVENT_IDS = [
  ...STAGE37_BATTLE_EVENT_IDS,
...
async function createStage27Completed(context: DebugScenarioContext): Promise<GameController> {
  const campaign = debugCampaign(context, "stage-27");
  const { Stage27Battle } = await import("./simulation/stage27-battle");
  const completedCampaign = new Stage27Battle(campaign, await stage27FullDeployment())
    .campaignSnapshot();
  const save: CompletedSaveData = {
    format: "ANGEL2-web-save",
    version: SAVE_VERSION,
...
async function createStage28Prebattle(context: DebugScenarioContext): Promise<GameController> {
  const controller = new GameController(context.difficulty);
  await controller.enterStage("stage-28", debugCampaign(context, "stage-28"));
  return controller;
}

async function createStage28Deployment(context: DebugScenarioContext): Promise<GameController> {
  const controller = new GameController(context.difficulty);
  await controller.enterStage(
    "stage-28",
    debugCampaign(context, "stage-28"),
    { preparation: true, statusMessage: "調試場景：瓦爾克麗守城戰部署。" },
  );
  return controller;
}

async function stage28FullDeployment() {
  const { STAGE28_DEFINITION } = await import("./content/stage28");
  return {
    placements: [
...
async function createStage28Opening(context: DebugScenarioContext): Promise<GameController> {
  const controller = await createStage28Deployment(context);
  controller.completeDeployment(await stage28FullDeployment());
  return controller;
}

async function createStage28Player(context: DebugScenarioContext): Promise<GameController> {
  const campaign = debugCampaign(context, "stage-28");
  const { Stage28Battle } = await import("./simulation/stage28-battle");
  const battle = new Stage28Battle(campaign, await stage28FullDeployment());
  const nia = battle.unit("1:0");
  if (!nia) throw new Error("stage 28 debug scenario is missing Nia");
  battle.focusId = nia.id;
  const battleCampaign = battle.campaignSnapshot();
  const save: BattleSaveData = {
    ...battleSaveBase(battleCampaign, "stage-28"),
    stageLabel: "保衛瓦爾克麗城",
    roster: battleCampaign.roster,
    consumedEventIds: [...STAGE28_BATTLE_EVENT_IDS],
    battle: {
      phase: "player",
      ...battle.serializableSnapshot(),
      cursor: { x: nia.x, y: nia.y },
  ...
  const controller = await GameController.fromSave(save, 1);
  controller.statusMessage = "調試場景：二十九人瓦爾克麗守城隊迎戰十七名攻城敵軍。";
  return controller;
}

async function createStage28Completed(context: DebugScenarioContext): Promise<GameController> {
  const campaign = debugCampaign(context, "stage-28");
  const { Stage28Battle } = await import("./simulation/stage28-battle");
  const completedCampaign = new Stage28Battle(campaign, await stage28FullDeployment())
    .campaignSnapshot();
  const save: CompletedSaveData = {
    format: "ANGEL2-web-save",
    version: SAVE_VERSION,
...
async function createStage29Prebattle(context: DebugScenarioContext): Promise<GameController> {
  const controller = new GameController(context.difficulty);
  await controller.enterStage("stage-29", debugCampaign(context, "stage-29"));
  return controller;
}

async function createStage29Deployment(context: DebugScenarioContext): Promise<GameController> {
  const controller = new GameController(context.difficulty);
  await controller.enterStage(
    "stage-29",
    debugCampaign(context, "stage-29"),
    { preparation: true, statusMessage: "調試場景：騎士城堡前部署。" },
  );
  return controller;
}

async function stage29FullDeployment() {
  const { STAGE29_DEFINITION } = await import("./content/stage29");
  const optionalSlots = [
...
async function createStage29Player(context: DebugScenarioContext): Promise<GameController> {
  const campaign = debugCampaign(context, "stage-29");
  const { Stage29Battle } = await import("./simulation/stage29-battle");
  const battle = new Stage29Battle(campaign, await stage29FullDeployment());
  const nia = battle.unit("1:0");
  if (!nia) throw new Error("stage 29 debug scenario is missing Nia");
  battle.focusId = nia.id;
  const battleCampaign = battle.campaignSnapshot();
  const save: BattleSaveData = {
    ...battleSaveBase(battleCampaign, "stage-29"),
    stageLabel: "騎士城堡前",
    roster: battleCampaign.roster,
    consumedEventIds: [...STAGE29_BATTLE_EVENT_IDS],
    battle: {
      phase: "player",
      ...battle.serializableSnapshot(),
      cursor: { x: nia.x, y: nia.y },
  ...
  const controller = await GameController.fromSave(save, 1);
  controller.statusMessage = "調試場景：十五人進攻隊迎戰騎士城堡十五名守軍。";
  return controller;
}

async function createStage29Completed(context: DebugScenarioContext): Promise<GameController> {
  const campaign = debugCampaign(context, "stage-29");
  const { Stage29Battle } = await import("./simulation/stage29-battle");
  const completedCampaign = new Stage29Battle(campaign, await stage29FullDeployment())
    .campaignSnapshot();
  const save: CompletedSaveData = {
    format: "ANGEL2-web-save",
    version: SAVE_VERSION,
...
async function createStage31Prebattle(context: DebugScenarioContext): Promise<GameController> {
  const controller = new GameController(context.difficulty);
  await controller.enterStage("stage-31", debugCampaign(context, "stage-31"));
  return controller;
}

async function createStage31Deployment(context: DebugScenarioContext): Promise<GameController> {
  const controller = new GameController(context.difficulty);
  await controller.enterStage(
    "stage-31",
    debugCampaign(context, "stage-31"),
    { preparation: true, statusMessage: "調試場景：前往斯德林海峽部署。" },
  );
  return controller;
}

async function stage31FullDeployment() {
  const { STAGE31_DEFINITION } = await import("./content/stage31");
  return {
    placements: [
...
async function createStage31Player(context: DebugScenarioContext): Promise<GameController> {
  const campaign = debugCampaign(context, "stage-31");
  const { Stage31Battle } = await import("./simulation/stage31-battle");
  const battle = new Stage31Battle(campaign, await stage31FullDeployment());
  const nia = battle.unit("1:0");
  if (!nia) throw new Error("stage 31 debug scenario is missing Nia");
  battle.focusId = nia.id;
  const battleCampaign = battle.campaignSnapshot();
  const save: BattleSaveData = {
    ...battleSaveBase(battleCampaign, "stage-31"),
    stageLabel: "前往斯德林海峽",
    roster: battleCampaign.roster,
    consumedEventIds: [...STAGE31_BATTLE_EVENT_IDS],
    battle: {
      phase: "player",
      ...battle.serializableSnapshot(),
      cursor: { x: nia.x, y: nia.y },
  ...
  const controller = await GameController.fromSave(save, 1);
  controller.statusMessage = "調試場景：十七人渡海隊迎戰菲伊魯茵十五名伏兵。";
  return controller;
}

async function createStage31Completed(context: DebugScenarioContext): Promise<GameController> {
  const campaign = debugCampaign(context, "stage-31");
  const { Stage31Battle } = await import("./simulation/stage31-battle");
  const completedCampaign = new Stage31Battle(campaign, await stage31FullDeployment())
    .campaignSnapshot();
  const save: CompletedSaveData = {
    format: "ANGEL2-web-save",
    version: SAVE_VERSION,
...
async function createStage32Deployment(context: DebugScenarioContext): Promise<GameController> {
  const controller = new GameController(context.difficulty);
  await controller.enterStage(
    "stage-32",
    debugCampaign(context, "stage-32"),
    { preparation: true, statusMessage: "調試場景：斯德林海峽聯軍戰部署。" },
  );
  return controller;
}

async function stage32FullDeployment() {
  const { STAGE32_DEFINITION } = await import("./content/stage32");
  return {
    placements: [
...
async function createStage32Player(context: DebugScenarioContext): Promise<GameController> {
  const campaign = debugCampaign(context, "stage-32");
  const { Stage32Battle } = await import("./simulation/stage32-battle");
  const battle = new Stage32Battle(campaign, await stage32FullDeployment());
  const nia = battle.unit("1:0");
  if (!nia) throw new Error("stage 32 debug scenario is missing Nia");
  battle.focusId = nia.id;
  const battleCampaign = battle.campaignSnapshot();
  const save: BattleSaveData = {
    ...battleSaveBase(battleCampaign, "stage-32"),
    stageLabel: "斯德林海峽",
    roster: battleCampaign.roster,
    consumedEventIds: [...STAGE32_BATTLE_EVENT_IDS],
    battle: {
      phase: "player",
      ...battle.serializableSnapshot(),
      cursor: { x: nia.x, y: nia.y },
  ...
  const controller = await GameController.fromSave(save, 1);
  controller.statusMessage = "調試場景：十六人追擊隊迎戰菲伊魯茵、芙瑪羅妮與十六名聯軍。";
  return controller;
}

async function createStage32Completed(context: DebugScenarioContext): Promise<GameController> {
  const campaign = debugCampaign(context, "stage-32");
  const { Stage32Battle } = await import("./simulation/stage32-battle");
  const completedCampaign = new Stage32Battle(campaign, await stage32FullDeployment())
    .campaignSnapshot();
  const save: CompletedSaveData = {
    format: "ANGEL2-web-save",
    version: SAVE_VERSION,
...
async function createStage33Deployment(context: DebugScenarioContext): Promise<GameController> {
  const controller = new GameController(context.difficulty);
  await controller.enterStage(
    "stage-33",
    debugCampaign(context, "stage-33"),
    { preparation: true, statusMessage: "調試場景：拉那洛城外部署。" },
  );
  return controller;
}

async function stage33FullDeployment() {
  const { STAGE33_DEFINITION } = await import("./content/stage33");
  return {
    placements: [
...
async function createStage33Player(context: DebugScenarioContext): Promise<GameController> {
  const campaign = debugCampaign(context, "stage-33");
  const { Stage33Battle } = await import("./simulation/stage33-battle");
  const battle = new Stage33Battle(campaign, await stage33FullDeployment());
  const nia = battle.unit("1:0");
  if (!nia) throw new Error("stage 33 debug scenario is missing Nia");
  battle.focusId = nia.id;
  const battleCampaign = battle.campaignSnapshot();
  const save: BattleSaveData = {
    ...battleSaveBase(battleCampaign, "stage-33"),
    stageLabel: "拉那洛城外",
    roster: battleCampaign.roster,
    consumedEventIds: [...STAGE33_BATTLE_EVENT_IDS],
    battle: {
      phase: "player",
      ...battle.serializableSnapshot(),
      cursor: { x: nia.x, y: nia.y },
  ...
  const controller = await GameController.fromSave(save, 1);
  controller.statusMessage = "調試場景：十人攻城隊迎戰二十九名拉那洛城外守軍。";
  return controller;
}

async function createStage33Completed(context: DebugScenarioContext): Promise<GameController> {
  const campaign = debugCampaign(context, "stage-33");
  const { Stage33Battle } = await import("./simulation/stage33-battle");
  const completedCampaign = new Stage33Battle(campaign, await stage33FullDeployment())
    .campaignSnapshot();
  const save: CompletedSaveData = {
    format: "ANGEL2-web-save",
    version: SAVE_VERSION,
...
async function createStage34Deployment(context: DebugScenarioContext): Promise<GameController> {
  const controller = new GameController(context.difficulty);
  await controller.enterStage(
    "stage-34",
    debugCampaign(context, "stage-34"),
    { preparation: true, statusMessage: "調試場景：拉那洛城內部署。" },
  );
  return controller;
}

async function stage34FullDeployment() {
  const { STAGE34_DEFINITION } = await import("./content/stage34");
  return {
    placements: [
...
async function createStage34Player(context: DebugScenarioContext): Promise<GameController> {
  const campaign = debugCampaign(context, "stage-34");
  const { Stage34Battle } = await import("./simulation/stage34-battle");
  const battle = new Stage34Battle(campaign, await stage34FullDeployment());
  const nia = battle.unit("1:0");
  if (!nia) throw new Error("stage 34 debug scenario is missing Nia");
  battle.focusId = nia.id;
  const battleCampaign = battle.campaignSnapshot();
  const save: BattleSaveData = {
    ...battleSaveBase(battleCampaign, "stage-34"),
    stageLabel: "拉那洛城內",
    roster: battleCampaign.roster,
    consumedEventIds: [...STAGE34_BATTLE_EVENT_IDS],
    battle: {
      phase: "player",
      ...battle.serializableSnapshot(),
      cursor: { x: nia.x, y: nia.y },
  ...
  const controller = await GameController.fromSave(save, 1);
  controller.statusMessage = "調試場景：十一人攻堅隊迎戰蕾娜吉芙與十八名城內敵軍。";
  return controller;
}

async function createStage34Completed(context: DebugScenarioContext): Promise<GameController> {
  const campaign = debugCampaign(context, "stage-34");
  const { Stage34Battle } = await import("./simulation/stage34-battle");
  const completedCampaign = new Stage34Battle(campaign, await stage34FullDeployment())
    .campaignSnapshot();
  const save: CompletedSaveData = {
    format: "ANGEL2-web-save",
    version: SAVE_VERSION,
...
async function createStage37Deployment(context: DebugScenarioContext): Promise<GameController> {
  const controller = new GameController(context.difficulty);
  await controller.enterStage(
    "stage-37",
    debugCampaign(context, "stage-37"),
    { preparation: true, statusMessage: "調試場景：究極女神決戰部署。" },
  );
  return controller;
}

async function stage37FullDeployment() {
  const { STAGE37_DEFINITION } = await import("./content/stage37");
  return {
    placements: [
...
async function createStage37Player(context: DebugScenarioContext): Promise<GameController> {
  const campaign = debugCampaign(context, "stage-37");
  const { Stage37Battle } = await import("./simulation/stage37-battle");
  const battle = new Stage37Battle(campaign, await stage37FullDeployment());
  const nia = battle.unit("1:0");
  if (!nia) throw new Error("stage 37 debug scenario is missing Nia");
  battle.focusId = nia.id;
  const battleCampaign = battle.campaignSnapshot();
  const save: BattleSaveData = {
    ...battleSaveBase(battleCampaign, "stage-37"),
    stageLabel: "究極女神",
    roster: battleCampaign.roster,
    consumedEventIds: [...STAGE37_BATTLE_EVENT_IDS],
    battle: {
      phase: "player",
      ...battle.serializableSnapshot(),
      cursor: { x: nia.x, y: nia.y },
  ...
  const controller = await GameController.fromSave(save, 1);
  controller.statusMessage = "調試場景：二十七人決戰隊迎戰究極女神的三個部位。";
  return controller;
}
...
const DEBUG_SCENARIO_FACTORIES = {
  ...
  "stage-28-prebattle": createStage28Prebattle,
  "stage-28-deployment": createStage28Deployment,
  "stage-28-opening": createStage28Opening,
  "stage-28-player": createStage28Player,
  "stage-28-near-victory": withSetup(createStage28Player, (controller) => {
    const nia = controller.battle.unit("1:0");
    const finalEnemy = controller.battle.unit("2:55");
    if (!nia || !finalEnemy) return;
    nia.x = 28;
    nia.y = 24;
    nia.experience = 0;
    ...
    controller.battle.focusId = nia.id;
    controller.cursor = { x: nia.x, y: nia.y };
    controller.cameraOrigin = { x: 23, y: 21 };
    controller.statusMessage = "調試場景：最後一名攻城魔劍戰士只剩 1 點生命。";
  }),
  "stage-28-near-defeat": withSetup(createStage28Player, (controller) => {
    const nia = controller.battle.unit("1:0");
    const enemy = controller.battle.unit("2:55");
    if (!nia || !enemy) return;
    nia.x = 28;
    nia.y = 24;
    nia.life = 1;
  ...
  "stage-28-victory-ready": withSetup(createStage28Player, (controller) => {
    controller.forceVictoryForTest();
  }),
  "stage-28-cleared": createStage28Completed,
  "stage-29-prebattle": createStage29Prebattle,
  "stage-29-deployment": createStage29Deployment,
  "stage-29-player": createStage29Player,
  "stage-29-near-victory": withSetup(createStage29Player, (controller) => {
    const nia = controller.battle.unit("1:0");
    const finalEnemy = controller.battle.unit("2:47");
    if (!nia || !finalEnemy) return;
    nia.x = 44;
    nia.y = 26;
    nia.experience = 0;
    ...
    controller.battle.focusId = nia.id;
    controller.cursor = { x: nia.x, y: nia.y };
    controller.cameraOrigin = { x: 36, y: 23 };
    controller.statusMessage = "調試場景：最後一名魔弓兵只剩 1 點生命。";
  }),
  "stage-29-near-defeat": withSetup(createStage29Player, (controller) => {
    const nia = controller.battle.unit("1:0");
    const enemy = controller.battle.unit("2:4");
    if (!nia || !enemy) return;
    nia.x = 44;
    nia.y = 26;
    nia.life = 1;
    ...
    controller.cursor = { x: nia.x, y: nia.y };
    controller.cameraOrigin = { x: 36, y: 23 };
    controller.statusMessage = "調試場景：妮雅只剩 1 點生命，艾西柯羅位於相鄰格。";
  }),
  "stage-29-victory-ready": withSetup(createStage29Player, (controller) => {
    controller.forceVictoryForTest();
  }),
  ...
  "stage-31-prebattle": createStage31Prebattle,
  "stage-31-deployment": createStage31Deployment,
  "stage-31-player": createStage31Player,
  "stage-31-near-victory": withSetup(createStage31Player, (controller) => {
    const nia = controller.battle.unit("1:0");
    const feiluyin = controller.battle.unit("2:5");
    if (!nia || !feiluyin) return;
    nia.x = 25;
    nia.y = 24;
    nia.life = controller.battle.statsFor(nia).maxLife;
    nia.acted = false;
    ...
    controller.battle.focusId = nia.id;
    controller.cursor = { x: nia.x, y: nia.y };
    controller.cameraOrigin = { x: 21, y: 21 };
    controller.statusMessage = "調試場景：最後的菲伊魯茵只剩 1 點生命。";
  }),
  "stage-31-near-defeat": withSetup(createStage31Player, (controller) => {
    const nia = controller.battle.unit("1:0");
    const feiluyin = controller.battle.unit("2:5");
    if (!nia || !feiluyin) return;
    nia.x = 25;
    ...
    controller.cursor = { x: nia.x, y: nia.y };
    controller.cameraOrigin = { x: 21, y: 21 };
    controller.statusMessage = "調試場景：妮雅只剩 1 點生命，菲伊魯茵位於相鄰格。";
  }),
  "stage-31-victory-ready": withSetup(createStage31Player, (controller) => {
    controller.forceVictoryForTest();
  }),
  "stage-31-cleared": createStage31Completed,
  "stage-32-deployment": createStage32Deployment,
  "stage-32-player": createStage32Player,
  "stage-32-near-victory": withSetup(createStage32Player, (controller) => {
    const nia = controller.battle.unit("1:0");
    const feiluyin = controller.battle.unit("2:5");
    if (!nia || !feiluyin) return;
    nia.x = 25;
  ...
  "stage-32-near-defeat": withSetup(createStage32Player, (controller) => {
    const nia = controller.battle.unit("1:0");
    const feiluyin = controller.battle.unit("2:5");
    if (!nia || !feiluyin) return;
    nia.x = 25;
  ...
  "stage-32-victory-ready": withSetup(createStage32Player, (controller) => {
    controller.forceVictoryForTest();
  }),
  "stage-32-cleared": createStage32Completed,
  "stage-33-deployment": createStage33Deployment,
  "stage-33-player": createStage33Player,
  "stage-33-near-victory": withSetup(createStage33Player, (controller) => {
    const nia = controller.battle.unit("1:0");
    const lastGuard = controller.battle.unit("2:55");
    if (!nia || !lastGuard) return;
    nia.x = 27;
    nia.y = 38;
    nia.life = controller.battle.statsFor(nia).maxLife;
    nia.experience = 0;
    ...
    lastGuard.life = 1;
    controller.battle.units = controller.battle.units.filter(
      ({ side, id }) => side === 1 || id === lastGuard.id,
    );
    for (const ally of controller.battle.units.filter(({ side, id }) => side === 1 && id !== nia.id)) {
      ally.acted = true;
    }
    controller.battle.focusId = nia.id;
    controller.cursor = { x: nia.x, y: nia.y };
    controller.cameraOrigin = { x: 23, y: 35 };
    controller.statusMessage = "調試場景：最後一名城外守軍只剩 1 點生命。";
  }),
  "stage-33-near-defeat": withSetup(createStage33Player, (controller) => {
    const nia = controller.battle.unit("1:0");
    const lastGuard = controller.battle.unit("2:55");
    if (!nia || !lastGuard) return;
    nia.x = 27;
    ...
    controller.cursor = { x: nia.x, y: nia.y };
    controller.cameraOrigin = { x: 23, y: 35 };
    controller.statusMessage = "調試場景：妮雅只剩 1 點生命，城外守軍位於相鄰格。";
  }),
  "stage-33-victory-ready": withSetup(createStage33Player, (controller) => {
    controller.forceVictoryForTest();
  }),
  "stage-33-cleared": createStage33Completed,
  "stage-34-deployment": createStage34Deployment,
  "stage-34-player": createStage34Player,
  "stage-34-near-victory": withSetup(createStage34Player, (controller) => {
    const nia = controller.battle.unit("1:0");
    const lastEnemy = controller.battle.unit("2:7");
    ...
    controller.battle.focusId = nia.id;
    controller.cursor = { x: nia.x, y: nia.y };
    controller.cameraOrigin = { x: 26, y: 14 };
    controller.statusMessage = "調試場景：最後的蕾娜吉芙只剩 1 點生命。";
  }),
  "stage-34-near-defeat": withSetup(createStage34Player, (controller) => {
    const nia = controller.battle.unit("1:0");
    const lastEnemy = controller.battle.unit("2:7");
    if (!nia || !lastEnemy) return;
    nia.x = 30;
    ...
    controller.cursor = { x: nia.x, y: nia.y };
    controller.cameraOrigin = { x: 26, y: 14 };
    controller.statusMessage = "調試場景：妮雅只剩 1 點生命，蕾娜吉芙位於相鄰格。";
  }),
  "stage-34-victory-ready": withSetup(createStage34Player, (controller) => {
    controller.forceVictoryForTest();
  }),
  ...
  "stage-37-near-defeat": withSetup(createStage37Player, (controller) => {
    const nia = controller.battle.unit("1:0");
    if (!nia) return;
    nia.life = 1;
    controller.battle.focusId = nia.id;
  ...
  "stage-37-victory-ready": withSetup(createStage37Player, (controller) => {
    controller.forceVictoryForTest();
  }),
