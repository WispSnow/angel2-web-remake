...
// 角色描述符按原版槽查表，槽号随后由 `REMAKE-108` 改写；四个被换走的槽都是 `FFh`
// 通用条目，与换进来的 51–54 同为通用身份，所以查表顺序不影响姓名与肖像。
export const STAGE2_SEMANTIC_ALLIED_UNITS = withSwappedGenericAllySlots(
  STAGE2_ALLIED_UNITS.map((unit) => {
