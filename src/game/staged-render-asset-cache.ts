...
interface ActiveStagedRenderAssets {
  ...
  readonly decodeImage: (source: string, originalUrl: string) => Promise<HTMLImageElement>;
  readonly createImageBitmap?: ImageBitmapDecoder;
  released: boolean;
}
...
export function activateStagedRenderAssets(
  ...
  const next: ActiveStagedRenderAssets = {
    ...
    blobConstructor,
    decodeImage,
    createImageBitmap: imageBitmapDecoder(bitmapScope),
    released: false,
  };
...
function decodeStagedImage(
  assets: ActiveStagedRenderAssets,
  entry: StagedRenderAssetEntry,
  url: string,
): Promise<HTMLImageElement> {
  return retryRefusedImageDecode({
    attempt: () => assets.decodeImage(entrySource(assets, entry), url),
    // The lease's own bytes, not a refetch and not the object URL.
    decodesElsewhere: () => decodesOutsideCompositor(
      assets.createImageBitmap,
      () => encodedBlob(assets, entry),
    ),
    // Releasing the lease revoked the object URL, so another attempt could
    // only fail on the dead `blob:`.
    abandoned: () => assets.released,
    failure: ({ outcome, reason, attempts, cause }) => new Error(
      outcome === "abandoned"
        ? `staged render asset lease ended before ${url} decoded`
        : outcome === "exhausted"
          ? `staged render asset failed to decode after ${attempts} attempts: ${url}: ${reason}`
          : `staged render asset failed to decode: ${url}: ${reason}`,
      { cause },
    ),
  });
}
